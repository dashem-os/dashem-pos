...
test('gives the Owner a real commercial plan catalog before tenant contracting', async () => {
  ...
  assert.match(workspace, /Capabilities contratadas/)
  assert.match(workspace, /Limites contratados/)
  assert.match(workspace, /capabilities incluídas/)
  assert.match(workspace, /setUsers\(String\(plan\.user_limit/)
  ...
  assert.match(provisioning, /users: String\(plan\.user_limit/)
