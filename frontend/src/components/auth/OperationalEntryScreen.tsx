  ...
  return <main className="flex min-h-[100dvh] items-center justify-center bg-[#06101f] p-4">
    <form autoComplete="off" onSubmit={mode === 'LOGIN' ? submitLogin : submitActivation} className="w-full max-w-md rounded-[28px] bg-white p-5 shadow-2xl sm:p-7">
        ...
        <div className="mt-5 grid grid-cols-2 rounded-xl bg-slate-100 p-1">
          <button type="button" aria-pressed={mode === 'LOGIN'} onClick={() => switchMode('LOGIN')} className={`min-h-11 rounded-lg border text-xs font-black transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-ink ${mode === 'LOGIN' ? 'border-slate-300 bg-white text-slate-950 shadow-sm' : 'border-transparent bg-slate-200/70 text-slate-600 hover:bg-slate-200 hover:text-slate-900'}`}>Entrar no turno</button>
          <button type="button" aria-pressed={mode === 'ACTIVATE'} onClick={() => switchMode('ACTIVATE')} className={`min-h-11 rounded-lg border text-xs font-black transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-ink ${mode === 'ACTIVATE' ? 'border-slate-300 bg-white text-slate-950 shadow-sm' : 'border-transparent bg-slate-200/70 text-slate-600 hover:bg-slate-200 hover:text-slate-900'}`}>Primeiro acesso / novo PIN</button>
        </div>
