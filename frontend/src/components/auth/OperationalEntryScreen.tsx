...
export function OperationalEntryScreen() {
  ...
  if (!context) return <main className="flex min-h-screen items-center justify-center bg-[#06101f] p-5">
    <section className="w-full max-w-lg rounded-[28px] bg-white p-7 text-center shadow-2xl sm:p-9">
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2"><button onClick={() => setTerminalCheck(value => value + 1)} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl border border-slate-300 font-black text-slate-800 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand/40"><RefreshCw className="h-4 w-4" />Tentar novamente</button><button onClick={() => navigateTo(session ? '/manage' : '/login')} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-slate-950 font-black text-white focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand/40"><ArrowLeft className="h-4 w-4" />{session ? 'Voltar à Gestão' : 'Entrar como gestor'}</button></div>
    </section>
  </main>
  ...
  return <main className="flex min-h-[100dvh] items-center justify-center bg-[#06101f] p-4">
    <form autoComplete="off" onSubmit={mode === 'LOGIN' ? submitLogin : submitActivation} className="w-full max-w-md rounded-[28px] bg-white p-5 shadow-2xl sm:p-7">
        ...
        <div className="mt-5 grid grid-cols-2 rounded-xl bg-slate-100 p-1">
          <button type="button" aria-pressed={mode === 'LOGIN'} onClick={() => switchMode('LOGIN')} className={`min-h-11 rounded-lg text-xs font-black focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand/40 ${mode === 'LOGIN' ? 'bg-white text-slate-950 shadow-sm' : 'text-slate-700'}`}>Entrar no turno</button>
          <button type="button" aria-pressed={mode === 'ACTIVATE'} onClick={() => switchMode('ACTIVATE')} className={`min-h-11 rounded-lg text-xs font-black focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand/40 ${mode === 'ACTIVATE' ? 'bg-white text-slate-950 shadow-sm' : 'text-slate-700'}`}>Primeiro acesso / novo PIN</button>
        </div>
        <label className="mt-5 block text-xs font-black uppercase tracking-wide text-slate-700">Código do colaborador<div className="mt-2 flex h-12 items-center rounded-xl border border-slate-300 px-3 focus-within:border-brand focus-within:ring-4 focus-within:ring-brand/20"><UserRound className="h-5 w-5 text-slate-500" /><input name="operational-identity" autoComplete="off" autoCapitalize="characters" autoCorrect="off" spellCheck={false} data-1p-ignore data-lpignore="true" value={employeeCode} onChange={event => setEmployeeCode(normalizeEmployeeCode(event.target.value))} placeholder="Ex.: ATD01" className="h-full min-w-0 flex-1 px-3 font-black uppercase outline-none" /></div></label>
        {mode === 'ACTIVATE' && <label className="mt-4 block text-xs font-black uppercase tracking-wide text-slate-700">Código temporário de ativação<input name="operational-activation" type="text" inputMode="numeric" autoComplete="one-time-code" data-1p-ignore data-lpignore="true" value={activationCode} onChange={event => setActivationCode(event.target.value.replace(/\D/g, '').slice(0, 8))} placeholder="8 números" className="mt-2 h-12 w-full rounded-xl border border-slate-300 px-4 font-mono text-lg font-black tracking-[.18em] text-slate-950 outline-none focus:border-brand focus:ring-4 focus:ring-brand/20" /></label>}
        {mode === 'LOGIN' ? <>
          <label className="mt-4 block text-xs font-black uppercase tracking-wide text-slate-700">PIN pessoal<input aria-label="PIN pessoal" name="operational-pin" type="text" inputMode="numeric" autoComplete="one-time-code" data-1p-ignore data-lpignore="true" value={pin} onChange={event => setPin(event.target.value.replace(/\D/g, '').slice(0, 8))} placeholder="4 a 8 números" className="operational-pin-mask mt-2 h-12 w-full rounded-xl border border-slate-300 bg-slate-50 px-4 text-center text-xl font-black tracking-[.35em] text-slate-950 outline-none focus:border-brand focus:ring-4 focus:ring-brand/20" /><span className="sr-only" aria-live="polite">{pin.length} dígitos informados</span></label>
        </> : <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <label className="text-xs font-black uppercase tracking-wide text-slate-700">Novo PIN<input name="operational-new-pin" type="text" inputMode="numeric" autoComplete="one-time-code" data-1p-ignore data-lpignore="true" value={pin} onChange={event => setPin(event.target.value.replace(/\D/g, '').slice(0, 8))} className="operational-pin-mask mt-2 h-12 w-full rounded-xl border border-slate-300 px-4 text-center text-lg font-black tracking-[.25em] text-slate-950 outline-none focus:border-brand focus:ring-4 focus:ring-brand/20" /></label>
