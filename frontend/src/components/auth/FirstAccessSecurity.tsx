...
import { Check, Copy, Eye, EyeOff, KeyRound, Loader2, LogOut, ShieldCheck } from 'lucide-react'
  ...
  return (
    <SecurityShell title={recovery ? 'Defina uma nova senha' : 'Crie sua senha de acesso'} description="Sua senha é protegida pelo Supabase Auth e nunca é armazenada pelo Dashem POS.">
      <form onSubmit={submit} className="mt-8 space-y-5">
        <label className="block text-sm font-bold">Nova senha<div className="mt-2 flex h-12 w-full items-center rounded-xl border border-slate-300 px-4 focus-within:border-brand-ink focus-within:ring-4 focus-within:ring-brand-soft"><input type={showPassword ? 'text' : 'password'} autoComplete="new-password" value={password} onChange={e => setPassword(e.target.value)} className="h-full min-w-0 flex-1 bg-transparent text-slate-900 outline-none placeholder:text-slate-400" /><button type="button" onClick={() => setShowPassword(value => !value)} className="flex h-10 w-10 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700" aria-label={showPassword ? 'Ocultar senha' : 'Mostrar senha'}>{showPassword ? <EyeOff className="h-5 w-5" /> : <Eye className="h-5 w-5" />}</button></div></label>
        <label className="block text-sm font-bold">Confirme a senha<div className="mt-2 flex h-12 w-full items-center rounded-xl border border-slate-300 px-4 focus-within:border-brand-ink focus-within:ring-4 focus-within:ring-brand-soft"><input type={showPassword ? 'text' : 'password'} autoComplete="new-password" value={confirmation} onChange={e => setConfirmation(e.target.value)} className="h-full min-w-0 flex-1 bg-transparent text-slate-900 outline-none placeholder:text-slate-400" /><button type="button" onClick={() => setShowPassword(value => !value)} className="flex h-10 w-10 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700" aria-label={showPassword ? 'Ocultar senha' : 'Mostrar senha'}>{showPassword ? <EyeOff className="h-5 w-5" /> : <Eye className="h-5 w-5" />}</button></div></label>
    ...
    <SecurityShell title={enrollment ? 'Proteja o Console Owner' : 'Confirme seu segundo fator'} description={enrollment ? 'Escaneie o QR code com seu aplicativo autenticador. O acesso administrativo exige MFA.' : 'Digite o código atual do seu aplicativo autenticador para continuar.'}>
      {loading && !factorId ? <div className="flex justify-center py-12"><Loader2 className="h-7 w-7 animate-spin text-brand-ink" /></div> : (
        <form onSubmit={verify} className="mt-8">
          ...
          <label className="block text-sm font-bold">Código de 6 dígitos<input inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={code} onChange={e => setCode(e.target.value.replace(/\D/g, ''))} className="mt-2 h-14 w-full rounded-xl border border-slate-300 px-4 text-center font-mono text-2xl font-black tracking-[.45em] text-slate-900 outline-none focus:border-brand-ink focus:ring-4 focus:ring-brand-soft" /></label>
