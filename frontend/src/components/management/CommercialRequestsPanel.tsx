...
export function CommercialRequestsPanel() {
  ...
  return <section className="rounded-3xl border border-dashem-border bg-dashem-surface p-6">
    ...
    {storage && <div className="mt-5 rounded-xl border border-dashem-border p-4"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-black text-white">Armazenamento contratado</p><div className="flex flex-wrap gap-2"><span className="rounded-full bg-dashem-bg px-2 py-1 text-[10px] font-black text-slate-200">{measurementLabel[storage.measurement_status]}</span><span className="rounded-full bg-dashem-bg px-2 py-1 text-[10px] font-black text-slate-200">{quotaLabel[storage.quota_status]}</span></div></div><p className="mt-3 text-xs font-bold text-slate-200">Uso observado: {formatBytes(storage.used_bytes)} · reservado: {formatBytes(storage.reserved_bytes)} · disponível: {formatBytes(storage.available_bytes)}</p></div>}
