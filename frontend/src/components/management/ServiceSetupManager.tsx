...
import { Armchair, Ban, CalendarClock, CheckCircle2, LayoutGrid, MapPinned, MoreHorizontal, Pencil, Plus, RefreshCw, Users, XCircle } from 'lucide-react'
...
type Dialog = 'AREA' | 'TABLE' | 'EDIT_AREA' | 'EDIT_TABLE' | 'RESERVATION' | null
...
export function ServiceSetupManager() {
  ...
  const [areaForm, setAreaForm] = useState({ code: '', name: '', kind: 'INTERNAL' as api.ServiceArea['kind'], sort_order: '100' })
  const [tableForm, setTableForm] = useState({ code: '', name: '', capacity: '4', area_id: '', sort_order: '100' })
  const [editingArea, setEditingArea] = useState<api.ServiceArea | null>(null)
  const [editingTable, setEditingTable] = useState<api.ServiceTableProjection | null>(null)
  const [areaEditForm, setAreaEditForm] = useState({ name: '', kind: 'INTERNAL' as api.ServiceArea['kind'], sort_order: '100', reason: '' })
  const [tableEditForm, setTableEditForm] = useState({ name: '', capacity: '4', area_id: '', sort_order: '100', reason: '' })
  ...
  const createArea = async (event: React.FormEvent) => {
    ...
    try {
      await api.createServiceArea(headers, { store_id: store.id, code: areaForm.code, name: areaForm.name, kind: areaForm.kind, sort_order: Number(areaForm.sort_order), actor_id: operatorId })
      setAreaForm({ code: '', name: '', kind: 'INTERNAL', sort_order: '100' }); setDialog(null); showToast('success', 'Ambiente cadastrado.'); await load()
  ...
  const createTable = async (event: React.FormEvent) => {
    ...
    try {
      await api.createServiceTable(headers, crypto.randomUUID(), {
        ...
        area_id: tableForm.area_id || undefined, sort_order: Number(tableForm.sort_order), actor_id: operatorId,
      })
      setTableForm({ code: '', name: '', capacity: '4', area_id: '', sort_order: '100' }); setDialog(null); showToast('success', 'Mesa adicionada ao mapa.'); await load()
  ...
  const openAreaEditor = (area: api.ServiceArea) => {
    setEditingArea(area)
    setAreaEditForm({ name: area.name, kind: area.kind, sort_order: String(area.sort_order), reason: '' })
    setDialog('EDIT_AREA')
  }

  const updateArea = async (event: React.FormEvent) => {
    event.preventDefault(); if (!editingArea || areaEditForm.reason.trim().length < 3) return; setBusy(true)
    try {
      await api.updateServiceArea(headers, editingArea.id, {
        name: areaEditForm.name, kind: areaEditForm.kind, sort_order: Number(areaEditForm.sort_order),
        reason: areaEditForm.reason.trim(), actor_id: operatorId,
      })
      setDialog(null); setEditingArea(null); showToast('success', 'Ambiente atualizado e reordenado.'); await load()
    } catch (reason) { showToast('error', reason instanceof Error ? reason.message : 'Falha ao atualizar ambiente.') }
    finally { setBusy(false) }
  }

  const openTableEditor = (table: api.ServiceTableProjection) => {
    setEditingTable(table)
    setTableEditForm({ name: table.name, capacity: String(table.capacity), area_id: table.area_id || '', sort_order: String(table.sort_order), reason: '' })
    setDialog('EDIT_TABLE')
  }

  const updateTable = async (event: React.FormEvent) => {
    event.preventDefault(); if (!editingTable || tableEditForm.reason.trim().length < 3) return; setBusy(true)
    try {
      await api.updateServiceTable(headers, editingTable.id, {
        expected_version: editingTable.version, name: tableEditForm.name, capacity: Number(tableEditForm.capacity),
        area_id: tableEditForm.area_id, sort_order: Number(tableEditForm.sort_order),
        reason: tableEditForm.reason.trim(), actor_id: operatorId,
      })
      setDialog(null); setEditingTable(null); showToast('success', 'Mesa atualizada no mapa.'); await load()
    } catch (reason) { showToast('error', reason instanceof Error ? reason.message : 'Falha ao atualizar mesa.') }
    finally { setBusy(false) }
  }
  ...
  return <div className="space-y-6">
    ...
    <Modal isOpen={dialog === 'AREA'} onClose={() => setDialog(null)} title="Novo ambiente" subtitle="Organize salão, área externa, balcão ou retirada."><form onSubmit={createArea} className="space-y-4"><Field label="Código" value={areaForm.code} onChange={(value) => setAreaForm({ ...areaForm, code: value })} placeholder="SALAO" /><Field label="Nome" value={areaForm.name} onChange={(value) => setAreaForm({ ...areaForm, name: value })} placeholder="Salão principal" /><div className="grid gap-3 sm:grid-cols-2"><label className="block text-xs font-black text-dashem-strong">Tipo<select value={areaForm.kind} onChange={(event) => setAreaForm({ ...areaForm, kind: event.target.value as api.ServiceArea['kind'] })} className="mt-2 h-11 w-full rounded-xl border border-dashem-border bg-dashem-surface-elevated px-3 text-sm text-dashem-strong">{Object.entries(kindLabel).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label><Field label="Ordem no mapa" type="number" value={areaForm.sort_order} onChange={(value) => setAreaForm({ ...areaForm, sort_order: value })} /></div><Submit busy={busy} label="Cadastrar ambiente" /></form></Modal>
    <Modal isOpen={dialog === 'TABLE'} onClose={() => setDialog(null)} title="Nova mesa" subtitle="Configuração exclusiva da retaguarda."><form onSubmit={createTable} className="space-y-4"><div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><Field label="Código" value={tableForm.code} onChange={(value) => setTableForm({ ...tableForm, code: value })} placeholder="MESA-01" /><Field label="Nome visível" value={tableForm.name} onChange={(value) => setTableForm({ ...tableForm, name: value })} placeholder="Mesa 01" /></div><div className="grid gap-3 sm:grid-cols-2"><Field label="Capacidade" type="number" value={tableForm.capacity} onChange={(value) => setTableForm({ ...tableForm, capacity: value })} /><Field label="Ordem no ambiente" type="number" value={tableForm.sort_order} onChange={(value) => setTableForm({ ...tableForm, sort_order: value })} /></div><label className="block text-xs font-black text-dashem-strong">Ambiente<select required value={tableForm.area_id} onChange={(event) => setTableForm({ ...tableForm, area_id: event.target.value })} className="mt-2 h-11 w-full rounded-xl border border-dashem-border bg-dashem-surface-elevated px-3 text-sm text-dashem-strong"><option value="">Selecione</option>{areas.map((area) => <option key={area.id} value={area.id}>{area.name}</option>)}</select></label><Submit busy={busy} label="Adicionar ao mapa" /></form></Modal>
    <Modal isOpen={dialog === 'EDIT_AREA'} onClose={() => { setDialog(null); setEditingArea(null) }} title="Editar ambiente" subtitle={`O código ${editingArea?.code || ''} permanece como identidade de auditoria.`}><form onSubmit={updateArea} className="space-y-4"><Field label="Nome" value={areaEditForm.name} onChange={(value) => setAreaEditForm({ ...areaEditForm, name: value })} /><div className="grid gap-3 sm:grid-cols-2"><label className="block text-xs font-black text-dashem-strong">Tipo<select value={areaEditForm.kind} onChange={(event) => setAreaEditForm({ ...areaEditForm, kind: event.target.value as api.ServiceArea['kind'] })} className="mt-2 h-11 w-full rounded-xl border border-dashem-border bg-dashem-surface-elevated px-3 text-sm text-dashem-strong">{Object.entries(kindLabel).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label><Field label="Ordem no mapa" type="number" value={areaEditForm.sort_order} onChange={(value) => setAreaEditForm({ ...areaEditForm, sort_order: value })} /></div><ReasonField value={areaEditForm.reason} onChange={(value) => setAreaEditForm({ ...areaEditForm, reason: value })} /><Submit busy={busy || areaEditForm.reason.trim().length < 3} label="Salvar ambiente" /></form></Modal>
    <Modal isOpen={dialog === 'EDIT_TABLE'} onClose={() => { setDialog(null); setEditingTable(null) }} title="Editar mesa" subtitle={`O código ${editingTable?.code || ''} permanece como identidade de auditoria.`}><form onSubmit={updateTable} className="space-y-4"><Field label="Nome visível" value={tableEditForm.name} onChange={(value) => setTableEditForm({ ...tableEditForm, name: value })} /><div className="grid gap-3 sm:grid-cols-2"><Field label="Capacidade" type="number" value={tableEditForm.capacity} onChange={(value) => setTableEditForm({ ...tableEditForm, capacity: value })} /><Field label="Ordem no ambiente" type="number" value={tableEditForm.sort_order} onChange={(value) => setTableEditForm({ ...tableEditForm, sort_order: value })} /></div><label className="block text-xs font-black text-dashem-strong">Ambiente<select required value={tableEditForm.area_id} onChange={(event) => setTableEditForm({ ...tableEditForm, area_id: event.target.value })} className="mt-2 h-11 w-full rounded-xl border border-dashem-border bg-dashem-surface-elevated px-3 text-sm text-dashem-strong"><option value="">Selecione</option>{areas.map((area) => <option key={area.id} value={area.id}>{area.name}</option>)}</select></label><ReasonField value={tableEditForm.reason} onChange={(value) => setTableEditForm({ ...tableEditForm, reason: value })} /><Submit busy={busy || tableEditForm.reason.trim().length < 3} label="Salvar mesa" /></form></Modal>
...
function ReasonField({ value, onChange }: { value: string; onChange: (value: string) => void }) { return <label className="block text-xs font-black text-dashem-strong">Motivo da alteração<textarea required minLength={3} value={value} onChange={(event) => onChange(event.target.value)} placeholder="Ex.: reorganização do salão para o turno da noite" className="mt-2 min-h-24 w-full resize-none rounded-xl border border-dashem-border bg-dashem-surface-elevated p-3 text-sm text-dashem-strong outline-none focus:border-dashem-red" /></label> }
