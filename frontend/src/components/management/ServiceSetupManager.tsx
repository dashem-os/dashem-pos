  ...
  return <div className="space-y-6">
    <section className="overflow-hidden rounded-3xl border border-dashem-border bg-dashem-surface">
      <div className="flex flex-col justify-between gap-5 p-6 lg:flex-row lg:items-end">
        <div><p className="text-[11px] font-black uppercase tracking-[.18em] text-orange-400">Estrutura do atendimento</p><h1 className="mt-2 text-3xl font-black text-white">Ambientes, mesas e reservas</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-dashem-muted">Configure aqui o mapa e a agenda. Na tela operacional a equipe abre mesas e comandas, confirma chegadas e sinaliza impedimentos.</p></div>
    ...
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
      ...
      <div className="flex flex-wrap gap-2">
        {canReserve && <button onClick={() => setDialog('RESERVATION')} disabled={!reservableTables.length} className="flex h-10 items-center gap-2 rounded-xl border border-orange-700/60 px-4 text-xs font-black text-orange-300 disabled:opacity-40"><CalendarClock className="h-4 w-4" />Nova reserva</button>}
    ...
    <Modal isOpen={dialog === 'RESERVATION'} onClose={() => setDialog(null)} title="Nova reserva" subtitle="A agenda aceita mesas livres ou ocupadas e impede conflito de horário."><form onSubmit={createReservation} className="space-y-4"><label className="block text-xs font-black text-white">Mesa<select required value={reservationForm.table_id} onChange={(event) => setReservationForm({ ...reservationForm, table_id: event.target.value })} className="mt-2 h-11 w-full rounded-xl border border-dashem-border bg-dashem-surface-elevated px-3 text-sm text-white"><option value="">Selecione</option>{reservableTables.map((table) => <option key={table.id} value={table.id}>{table.name} · {table.area} · {table.status}</option>)}</select></label><div className="grid grid-cols-2 gap-3"><Field label="Responsável" value={reservationForm.customer_name} onChange={(value) => setReservationForm({ ...reservationForm, customer_name: value })} /><Field label="Telefone" value={reservationForm.customer_phone} onChange={(value) => setReservationForm({ ...reservationForm, customer_phone: value })} /></div><div className="grid gap-3 sm:grid-cols-3"><Field label="Pessoas" type="number" value={reservationForm.party_size} onChange={(value) => setReservationForm({ ...reservationForm, party_size: value })} /><Field label="Data e hora" type="datetime-local" value={reservationForm.reserved_for} onChange={(value) => setReservationForm({ ...reservationForm, reserved_for: value })} /><Field label="Duração (min)" type="number" value={reservationForm.duration_minutes} onChange={(value) => setReservationForm({ ...reservationForm, duration_minutes: value })} /></div><Field label="Observação" value={reservationForm.notes} onChange={(value) => setReservationForm({ ...reservationForm, notes: value })} /><Submit busy={busy} label="Confirmar reserva" /></form></Modal>
    <Modal isOpen={Boolean(stateChange)} onClose={() => { setStateChange(null); setStateReason('') }} title={stateChange?.blocked ? 'Sinalizar impedimento' : 'Liberar mesa'} subtitle={stateChange?.blocked ? 'Use para defeito, manutenção ou impedimento real. Reservas possuem agenda própria.' : 'A justificativa mantém a alteração rastreável.'}><form onSubmit={(event) => { event.preventDefault(); if (stateChange) void setTableState(stateChange.table, stateChange.blocked, stateReason) }} className="space-y-4"><div className="rounded-xl bg-dashem-bg p-4 text-sm font-black text-white">{stateChange?.table.name}</div><label className="block text-xs font-black text-white">Motivo<textarea autoFocus required minLength={3} value={stateReason} onChange={(event) => setStateReason(event.target.value)} placeholder={stateChange?.blocked ? 'Ex.: cadeira danificada; aguardando manutenção' : 'Ex.: manutenção concluída'} className="mt-2 min-h-24 w-full resize-none rounded-xl border border-dashem-border bg-dashem-surface-elevated p-3 text-sm text-white outline-none focus:border-dashem-red" /></label><Submit busy={busy || stateReason.trim().length < 3} label={stateChange?.blocked ? 'Confirmar impedimento' : 'Confirmar liberação'} /></form></Modal>
  </div>
}
