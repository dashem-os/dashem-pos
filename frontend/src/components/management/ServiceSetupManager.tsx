...
export function ServiceSetupManager() {
  ...
  return <div className="space-y-6">
    ...
    <Modal isOpen={dialog === 'TABLE'} onClose={() => setDialog(null)} title="Nova mesa" subtitle="Configuração exclusiva da retaguarda."><form onSubmit={createTable} className="space-y-4"><div className="grid grid-cols-2 gap-3"><Field label="Código" value={tableForm.code} onChange={(value) => setTableForm({ ...tableForm, code: value })} placeholder="MESA-01" /><Field label="Nome visível" value={tableForm.name} onChange={(value) => setTableForm({ ...tableForm, name: value })} placeholder="Mesa 01" /></div><Field label="Capacidade" type="number" value={tableForm.capacity} onChange={(value) => setTableForm({ ...tableForm, capacity: value })} /><label className="block text-xs font-black text-dashem-strong">Ambiente<select required value={tableForm.area_id} onChange={(event) => setTableForm({ ...tableForm, area_id: event.target.value })} className="mt-2 h-11 w-full rounded-xl border border-dashem-border bg-dashem-surface-elevated px-3 text-sm text-dashem-strong"><option value="">Selecione</option>{areas.map((area) => <option key={area.id} value={area.id}>{area.name}</option>)}</select></label><Submit busy={busy} label="Adicionar ao mapa" /></form></Modal>
