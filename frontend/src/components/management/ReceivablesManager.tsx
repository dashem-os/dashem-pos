...
export const ReceivablesManager: React.FC = () => {
  ...
  const overdue = open.filter((item) => (parseApiDate(item.due_at)?.getTime() ?? Infinity) < Date.now()).reduce((sum, item) => sum + Number(item.balance), 0)
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-5 xl:grid-cols-[.9fr_1.5fr]"><div className="rounded-3xl border border-dashem-border bg-dashem-surface p-5"><div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-state-success" /><h2 className="font-black">Política do cliente</h2></div><label className="mt-5 block text-xs font-bold text-dashem-muted">Cliente<select value={customerId} onChange={(event) => setCustomerId(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-dashem-border bg-dashem-bg px-3 text-sm text-dashem-strong">{customers.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.cpf_cnpj || 'documento pendente'}</option>)}</select></label><div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3"><Field label="Limite" value={limit} onChange={setLimit} /><Field label="Prazo (dias)" value={terms} onChange={setTerms} /></div><label className="mt-4 flex items-center gap-2 text-xs font-bold"><input type="checkbox" checked={allowOverdue} onChange={(e) => setAllowOverdue(e.target.checked)} />Permitir novo crédito com atraso</label><label className="mt-3 flex items-center gap-2 text-xs font-bold text-state-warning"><input type="checkbox" checked={blocked} onChange={(e) => setBlocked(e.target.checked)} />Bloquear crediário</label>{policy && <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2 rounded-2xl bg-dashem-bg p-4 text-xs"><span className="text-dashem-muted">Exposição</span><strong className="text-right">{money(policy.exposure)}</strong><span className="text-dashem-muted">Disponível</span><strong className="text-right text-state-success">{money(policy.available)}</strong></div>}<button disabled={!customerId || saving} onClick={() => void save()} className="mt-5 h-11 w-full rounded-xl bg-dashem-red text-xs font-black disabled:opacity-50">{saving ? 'Salvando…' : 'Salvar política auditada'}</button></div>
    <div className="overflow-hidden rounded-3xl border border-dashem-border bg-dashem-surface"><div className="border-b border-dashem-border p-5"><h2 className="font-black">Títulos emitidos</h2><p className="text-xs text-dashem-muted">Selecione títulos do mesmo cliente para receber ou renegociar.</p>{chosen.length > 0 && <div className="mt-4 flex flex-wrap items-end gap-2 rounded-2xl bg-dashem-bg p-3"><label className="text-[10px] font-black uppercase text-dashem-muted">Forma<select value={method} onChange={(e) => setMethod(e.target.value as typeof method)} className="mt-1 block h-9 rounded-lg border border-dashem-border bg-dashem-surface px-2 text-xs text-dashem-strong"><option value="PIX">PIX</option><option value="CREDIT_CARD">Cartão crédito</option><option value="DEBIT_CARD">Cartão débito</option></select></label><button disabled={saving} onClick={() => void receive()} className="h-9 rounded-lg bg-state-success px-3 text-xs font-black">Receber {money(chosen.reduce((sum, item) => sum + Number(item.balance), 0))}</button><label className="text-[10px] font-black uppercase text-dashem-muted">Parcelas<input value={installments} onChange={(e) => setInstallments(e.target.value)} type="number" min="1" max="120" className="mt-1 block h-9 w-20 rounded-lg border border-dashem-border bg-dashem-surface px-2 text-xs text-dashem-strong" /></label><button disabled={saving} onClick={() => void renegotiate()} className="h-9 rounded-lg border border-brand px-3 text-xs font-black text-brand-ink">Criar acordo</button></div>}</div><div className="divide-y divide-dashem-border">{receivables.length === 0 ? <p className="p-8 text-center text-sm text-dashem-muted">Nenhum título emitido neste contexto.</p> : receivables.map((item) => { const customer = customers.find((value) => value.id === item.customer_id); const selectable = ['OPEN', 'PARTIALLY_PAID', 'OVERDUE'].includes(item.status); return <div key={item.id} className="grid gap-3 p-4 text-xs sm:grid-cols-[auto_1.2fr_.8fr_.8fr_.7fr]"><input aria-label={`Selecionar título ${item.id}`} type="checkbox" disabled={!selectable} checked={selected.includes(item.id)} onChange={(e) => setSelected((current) => e.target.checked ? [...current, item.id] : current.filter((id) => id !== item.id))} /><div><p className="font-black text-dashem-strong">{customer?.name || item.customer_id}</p><p className="mt-1 font-mono text-xs text-dashem-muted">{item.id} · {item.status}</p></div><Value label="Principal" value={money(Number(item.principal_amount))} /><Value label="Saldo" value={money(Number(item.balance))} accent /><Value label="Vencimento" value={formatApiDateTime(item.due_at, 'date')} /></div> })}</div></div></section>{error && <p className="rounded-xl border border-state-danger-border bg-state-danger-soft p-3 text-xs font-bold text-state-danger">{error}</p>}
  </div>
}

function Metric({ icon: Icon, label, value, danger = false }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string; danger?: boolean }) { return <div className="rounded-2xl border border-dashem-border bg-dashem-surface p-5"><Icon className={`h-5 w-5 ${danger ? 'text-state-danger' : 'text-dashem-red'}`} /><p className="mt-4 text-[10px] font-black uppercase tracking-wider text-dashem-muted">{label}</p><p className={`mt-1 text-2xl font-black ${danger ? 'text-state-danger' : 'text-dashem-strong'}`}>{value}</p></div> }
...
function Value({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) { return <div><p className="text-dashem-muted">{label}</p><p className={`mt-1 font-black ${accent ? 'text-state-warning' : ''}`}>{value}</p></div> }
