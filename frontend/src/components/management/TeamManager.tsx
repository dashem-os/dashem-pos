...
export function TeamManager() {
  ...
  return <div className="space-y-5">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-xs font-black uppercase tracking-[.16em] text-dashem-red">Administração do tenant</p><h1 className="mt-1 text-2xl font-black text-dashem-strong">Equipe e identidades</h1><p className="mt-1 max-w-3xl text-sm text-dashem-muted">O cadastro do funcionário existe antes do acesso. Gestores entram por e-mail; equipe operacional assume o turno com código e PIN.</p></div>{canManage && <button onClick={openGrantForm} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-dashem-red px-5 text-sm font-black text-brand-contrast"><Plus className="h-4 w-4" />Conceder acesso</button>}</div>
...
function Info({ children }: { children: React.ReactNode }) { return <p className="rounded-xl border border-state-info-border bg-state-info-soft p-3 text-xs leading-5 text-state-info">{children}</p> }
function AccessBadge({ mode }: { mode: AccessMode }) { return <span className={`rounded-full px-2 py-1 text-xs font-black ${mode === 'PIN' ? 'bg-brand-soft text-brand-ink' : 'bg-state-info-soft text-state-info'}`}>{mode === 'PIN' ? 'CÓDIGO + PIN' : 'E-MAIL'}</span> }
function Action({ onClick, disabled = false, tone, children }: { onClick: () => void; disabled?: boolean; tone: 'violet' | 'amber' | 'emerald' | 'neutral'; children: React.ReactNode }) { const color = { violet: 'border-brand-soft text-brand-ink', amber: 'border-state-warning-border text-state-warning', emerald: 'border-state-success-border text-state-success', neutral: 'border-dashem-border text-dashem-muted' }[tone]; return <button disabled={disabled} onClick={onClick} className={`flex items-center gap-1 rounded-lg border px-3 py-2 text-xs font-black ${color}`}>{children}</button> }
