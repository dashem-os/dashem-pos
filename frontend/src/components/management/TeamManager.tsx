...
export function TeamManager() {
  ...
  return <div className="space-y-5">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-xs font-black uppercase tracking-[.16em] text-dashem-red">Administração do tenant</p><h2 className="mt-1 text-2xl font-black text-dashem-strong">Equipe e identidades</h2><p className="mt-1 max-w-3xl text-sm text-dashem-muted">O cadastro do funcionário existe antes do acesso. Gestores entram por e-mail; equipe operacional assume o turno com código e PIN.</p></div>{canManage && <button onClick={() => setFormOpen(true)} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-dashem-red px-5 text-sm font-black text-white"><Plus className="h-4 w-4" />Conceder acesso</button>}</div>
...
function AccessBadge({ mode }: { mode: AccessMode }) { return <span className={`rounded-full px-2 py-1 text-[10px] font-black ${mode === 'PIN' ? 'bg-violet-50 text-violet-700' : 'bg-sky-50 text-sky-700'}`}>{mode === 'PIN' ? 'CÓDIGO + PIN' : 'E-MAIL'}</span> }
