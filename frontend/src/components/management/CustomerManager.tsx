...
export function CustomerManager() {
  ...
  return <div className="space-y-5">
    ...
    <section className="overflow-hidden rounded-2xl border border-dashem-border bg-dashem-surface"><DataTable
      rows={visible}
      rowKey={(customer) => customer.id}
      empty={<div className="p-12 text-center"><ShoppingBag className="mx-auto h-9 w-9 text-dashem-muted" /><p className="mt-3 text-sm font-bold text-dashem-muted">Nenhum cliente encontrado.</p></div>}
      columns={[
        ...
        { key: 'purchases', header: 'Compras', cell: (customer) => <div><p className="font-black text-dashem-strong">{purchaseCount(customer.id)} vendas</p><p className="mt-1 text-xs text-state-success">{totalRevenue(customer.id).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p></div> },
