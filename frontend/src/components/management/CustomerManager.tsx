...
export function CustomerManager() {
  ...
  return <div className="space-y-5">
    <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs font-black uppercase tracking-[.16em] text-dashem-red">Relacionamento</p><h1 className="mt-1 text-3xl font-black text-white">Clientes</h1><p className="mt-2 text-sm text-dashem-muted">Cadastro, contato e histórico comercial no mesmo contexto do tenant.</p></div>{canManage && <button onClick={startCreate} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-dashem-red px-5 text-sm font-black"><Plus className="h-4 w-4" />Novo cliente</button>}</header>
    ...
    <section className="overflow-hidden rounded-2xl border border-dashem-border bg-dashem-surface"><div className="overflow-x-auto"><table className="w-full min-w-[820px] text-left"><thead className="bg-dashem-surface-elevated text-[10px] font-black uppercase tracking-wider text-dashem-muted"><tr><th className="p-4">Cliente</th><th className="p-4">Contato</th><th className="p-4">Compras</th><th className="p-4">Relacionamento</th><th className="p-4" /></tr></thead><tbody className="divide-y divide-dashem-border">{visible.map(customer => <tr key={customer.id}><td className="p-4"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-dashem-bg text-dashem-red"><UserRound className="h-5 w-5" /></div><div><p className="font-black text-white">{customer.name}</p><p className="mt-1 font-mono text-[11px] text-dashem-muted">{customer.cpf_cnpj || 'Documento não informado'}</p></div></div></td><td className="p-4 text-xs text-slate-300"><p className="flex items-center gap-2"><Phone className="h-3.5 w-3.5" />{customer.phone || 'Não informado'}</p><p className="mt-2 flex items-center gap-2"><Mail className="h-3.5 w-3.5" />{customer.email || 'Não informado'}</p></td><td className="p-4"><p className="font-black text-white">{purchaseCount(customer.id)} vendas</p><p className="mt-1 text-xs text-emerald-400">{totalRevenue(customer.id).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p></td><td className="p-4 text-xs text-dashem-muted">Desde {new Date(customer.created_at).toLocaleDateString('pt-BR')}</td><td className="p-4">{canManage && <button onClick={() => startEdit(customer)} className="flex h-9 items-center gap-2 rounded-lg border border-dashem-border px-3 text-xs font-black text-slate-300"><Pencil className="h-3.5 w-3.5" />Editar</button>}</td></tr>)}</tbody></table></div>{visible.length === 0 && <div className="p-12 text-center"><ShoppingBag className="mx-auto h-9 w-9 text-slate-600" /><p className="mt-3 text-sm font-bold text-dashem-muted">Nenhum cliente encontrado.</p></div>}</section>
...
function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl border border-dashem-border bg-dashem-surface p-4"><p className="text-xs font-black uppercase text-dashem-muted">{label}</p><p className="mt-2 text-2xl font-black text-white">{value}</p></div> }
