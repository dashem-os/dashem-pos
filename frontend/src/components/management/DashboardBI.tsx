  ...
  return <div className="space-y-6">
    <section className="rounded-3xl border border-dashem-border bg-gradient-to-br from-dashem-surface to-[#14253f] p-6 shadow-xl">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div><p className="text-[11px] font-extrabold uppercase tracking-[.18em] text-dashem-red">Visão geral da unidade</p><h1 className="mt-2 text-3xl font-black text-white">{store?.name}</h1><p className="mt-2 text-sm text-dashem-muted">{tenant?.name} · projeção gerada em {formatProductDateTime(overview.generated_at)}</p></div>
      ...
      <div className={`mt-5 flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold ${overview.source_watermark && overview.projection_lag_seconds > 300 ? 'border-amber-800 bg-amber-950/30 text-amber-300' : 'border-emerald-800 bg-emerald-950/30 text-emerald-300'}`}><Database className="h-4 w-4" />{overview.source_watermark ? `Fonte persistida até ${formatProductDateTime(overview.source_watermark)} · atraso de ${overview.projection_lag_seconds}s.` : 'Nenhum fato operacional persistido no período; não há watermark de origem.'}</div>
    </section>
    ...
    {drilldown && <section className="rounded-3xl border border-dashem-border bg-dashem-surface p-6"><div className="flex justify-between"><div><p className="text-[10px] font-black uppercase tracking-wider text-dashem-red">Drill-down rastreável</p><h3 className="mt-1 font-black text-white">{new Date(`${drilldown.competence_date}T12:00:00`).toLocaleDateString('pt-BR')} · {drilldown.total} fontes</h3></div><button onClick={() => setDrilldown(null)} className="rounded-lg border border-dashem-border p-2"><X className="h-4 w-4" /></button></div><div className="mt-4 divide-y divide-dashem-border">{drilldown.items.map((item) => <div key={item.source_id} className="flex justify-between py-3 text-xs"><div><p className="font-black text-white">{item.source_type} · {item.source_id}</p><p className="text-dashem-muted">{formatProductDateTime(item.occurred_at)}</p></div><p className="font-black text-white">{formatCurrency(item.amount)}</p></div>)}</div></section>}
