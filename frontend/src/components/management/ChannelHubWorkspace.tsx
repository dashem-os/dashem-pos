...
export function ChannelHubWorkspace() {
  ...
  return <section className="space-y-5 text-dashem-strong">
    <header className="flex flex-col justify-between gap-4 rounded-2xl border border-dashem-border bg-dashem-surface p-6 text-dashem-strong sm:flex-row sm:items-center"><div><p className="text-xs font-black uppercase tracking-[.16em] text-state-info">Omnichannel · inbox durável</p><h1 className="mt-1 text-2xl font-black">Dashem Channel Hub</h1><p className="mt-2 max-w-2xl text-sm text-dashem-muted">Eventos externos entram no mesmo Order Engine. Conexão só aparece ativa após validação real do adapter.</p></div><div className="flex gap-2"><button onClick={() => void load()} className="flex h-11 items-center gap-2 rounded-xl border border-dashem-border px-4 text-xs font-black"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />Atualizar</button>{permissions.includes('channel.configure') && <button onClick={() => setFormOpen(true)} className="flex h-11 items-center gap-2 rounded-xl bg-state-info px-4 text-xs font-black"><Plus className="h-4 w-4" />Nova conexão</button>}</div></header>
    <div className="grid gap-5 xl:grid-cols-[420px_1fr]"><section className="rounded-3xl border border-dashem-border bg-white p-5"><h2 className="font-black">Conexões</h2><p className="text-xs text-dashem-muted">{connections.length} merchants persistidos</p><div className="mt-4 space-y-3">{loading && <Loader2 className="h-5 w-5 animate-spin" />}{!loading && connections.length === 0 && <Empty text="Nenhum canal configurado. iFood e 99Food não serão mostrados como conectados sem credenciais e validação." />}{connections.map((connection) => <article key={connection.id} className="rounded-2xl border border-dashem-border p-4"><div className="flex items-start justify-between"><div><p className="font-black">{connection.provider_code}</p><p className="text-xs text-dashem-muted">{connection.merchant_external_id}</p></div><span className={`rounded-full px-2 py-1 text-xs font-black ${connection.status === 'CONNECTED' ? 'bg-state-success-soft text-state-success' : 'bg-state-warning-soft text-state-warning'}`}>{connectionLabels[connection.status]}</span></div>{connection.last_error_code && <p className="mt-2 text-xs text-state-warning">{connection.last_error_code}</p>}{permissions.includes('channel.configure') && connection.status !== 'CONNECTED' && <button disabled={busy} onClick={() => void validate(connection.id)} className="mt-3 h-9 w-full rounded-xl border border-state-info-border text-xs font-black text-state-info">Validar com provider</button>}</article>)}</div></section>
      <section className="rounded-3xl border border-dashem-border bg-white p-5"><div className="flex items-center justify-between"><div><h2 className="font-black">External Order Inbox</h2><p className="text-xs text-dashem-muted">Persistência, ack, normalização e quarentena observáveis</p></div><Radio className="h-5 w-5 text-state-info" /></div><p className="mt-3 rounded-xl border border-state-info-border bg-state-info-soft p-3 text-xs leading-5 text-dashem-strong">Sem processamento contínuo hospedado: cada evento é aplicado quando chega, por um worker quando houver, ou pelo botão Retomar. Um evento parado não avança sozinho.</p><div className="mt-4 overflow-x-auto"><ResponsiveTable className="w-full text-left text-xs"><thead><tr className="border-b text-[10px] uppercase text-dashem-muted"><th className="p-3">Recebido</th><th className="p-3">Pedido externo</th><th className="p-3">Estado</th><th className="p-3">Order</th></tr></thead><tbody>{events.map((event) => <tr key={event.id} className="border-b border-dashem-border"><td className="p-3">{formatApiDateTime(event.received_at)}</td><td className="p-3 font-bold">{event.external_order_id}</td><td className="p-3"><span className="flex items-center gap-1 font-black"><InboxStatusIcon status={event.status} />{inboxStatusLabels[event.status] ?? event.status}</span>{event.quarantine_reason && <p className={`mt-1 max-w-xs text-xs ${resumableInbox.includes(event.status) ? 'text-state-warning' : 'text-dashem-muted'}`}>{event.quarantine_reason}</p>}{event.retention_until && <RetentionNote until={event.retention_until} />}{permissions.includes('channel.manage') && resumableInbox.includes(event.status) && <button disabled={busy} onClick={() => void resume(event.id)} className="mt-2 h-8 rounded-lg border border-state-info-border px-3 text-xs font-black text-state-info">Retomar</button>}</td><td className="p-3 font-mono text-xs">{event.order_id || '—'}</td></tr>)}</tbody></ResponsiveTable>{!loading && events.length === 0 && <Empty text="Nenhum evento externo recebido. A tela não injeta pedidos demonstrativos." />}</div></section></div>
...
function ConnectionDialog({ storeId, actorId, headers, onClose, onCreated, showToast }: { storeId: string; actorId: string; headers: Record<string, string>; onClose: () => void; onCreated: () => Promise<void>; showToast: (type: 'success' | 'error' | 'info', text: string) => void }) {
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setSaving(true); try { const result = await api.createMerchantConnection(headers, crypto.randomUUID(), { store_id: storeId, provider_code: form.provider, merchant_external_id: form.merchant, channel_name: form.name, credentials_ref: form.credentials || undefined, actor_id: actorId }); showToast('success', 'Conexão cadastrada como não validada.'); if (result.connection) await onCreated() } catch (error) { showToast('error', error instanceof Error ? error.message : 'Não foi possível cadastrar a conexão.') } finally { setSaving(false) } }
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-dashem-strong/70 p-4"><form onSubmit={submit} className="responsive-dialog w-full max-w-lg space-y-3 rounded-3xl bg-white p-6"><div className="flex justify-between"><div><p className="text-xs font-black uppercase text-state-info">Configuração persistida</p><h2 className="text-xl font-black">Nova conexão</h2></div><button type="button" onClick={onClose}>×</button></div><label className="block text-xs font-black">Provider<select value={form.provider} onChange={(e) => setForm({ ...form, provider: e.target.value })} className="mt-1 h-11 w-full rounded-xl border px-3"><option value="IFOOD">iFood</option><option value="99FOOD">99Food</option><option value="OTHER">Outro adapter</option></select></label><Field label="ID do merchant" value={form.merchant} onChange={(value) => setForm({ ...form, merchant: value })} /><Field label="Nome do canal" value={form.name} onChange={(value) => setForm({ ...form, name: value })} /><Field label="Referência segura das credenciais" value={form.credentials} onChange={(value) => setForm({ ...form, credentials: value })} placeholder="secret://tenant/provider" /><p className="text-xs leading-5 text-dashem-muted">Cadastrar não significa conectar. O status só muda após o adapter validar credenciais e merchant.</p><button disabled={saving} className="h-11 w-full rounded-xl bg-state-info text-sm font-black text-white">{saving ? 'Salvando...' : 'Cadastrar sem fingir conexão'}</button></form></div>
}
