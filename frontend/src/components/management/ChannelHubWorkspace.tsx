...
export function ChannelHubWorkspace() {
  ...
  return <section className="space-y-5 text-slate-950">
    ...
    <div className="grid gap-5 xl:grid-cols-[420px_1fr]"><section className="rounded-3xl border border-slate-200 bg-white p-5"><h2 className="font-black">Conexões</h2><p className="text-xs text-slate-500">{connections.length} merchants persistidos</p><div className="mt-4 space-y-3">{loading && <Loader2 className="h-5 w-5 animate-spin" />}{!loading && connections.length === 0 && <Empty text="Nenhum canal configurado. iFood e 99Food não serão mostrados como conectados sem credenciais e validação." />}{connections.map((connection) => <article key={connection.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between"><div><p className="font-black">{connection.provider_code}</p><p className="text-xs text-slate-500">{connection.merchant_external_id}</p></div><span className={`rounded-full px-2 py-1 text-xs font-black ${connection.status === 'CONNECTED' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-900'}`}>{connectionLabels[connection.status]}</span></div>{connection.last_error_code && <p className="mt-2 text-xs text-amber-800">{connection.last_error_code}</p>}{permissions.includes('channel.configure') && connection.status !== 'CONNECTED' && <button disabled={busy} onClick={() => void validate(connection.id)} className="mt-3 h-9 w-full rounded-xl border border-cyan-300 text-xs font-black text-cyan-800">Validar com provider</button>}</article>)}</div></section>
      <section className="rounded-3xl border border-slate-200 bg-white p-5"><div className="flex items-center justify-between"><div><h2 className="font-black">External Order Inbox</h2><p className="text-xs text-slate-500">Persistência, ack, normalização e quarentena observáveis</p></div><Radio className="h-5 w-5 text-cyan-600" /></div><div className="mt-4 overflow-x-auto"><table className="w-full text-left text-xs"><thead><tr className="border-b text-[10px] uppercase text-dashem-muted"><th className="p-3">Recebido</th><th className="p-3">Pedido externo</th><th className="p-3">Estado</th><th className="p-3">Order</th></tr></thead><tbody>{events.map((event) => <tr key={event.id} className="border-b border-slate-100"><td className="p-3">{new Date(event.received_at).toLocaleString('pt-BR')}</td><td className="p-3 font-bold">{event.external_order_id}</td><td className="p-3"><span className="flex items-center gap-1 font-black">{event.status === 'QUARANTINED' ? <AlertTriangle className="h-3.5 w-3.5 text-amber-600" /> : <CheckCircle2 className="h-3.5 w-3.5 text-emerald-600" />}{event.status}</span>{event.quarantine_reason && <p className="mt-1 max-w-xs text-xs text-amber-700">{event.quarantine_reason}</p>}</td><td className="p-3 font-mono text-xs">{event.order_id || '—'}</td></tr>)}</tbody></table>{!loading && events.length === 0 && <Empty text="Nenhum evento externo recebido. A tela não injeta pedidos demonstrativos." />}</div></section></div>
    <div className="grid gap-5 lg:grid-cols-2"><section className="rounded-3xl border border-slate-200 bg-white p-5"><h2 className="font-black">Catálogo por canal</h2><p className="text-xs text-slate-500">Identidade canônica e publicação item a item</p><div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3"><Metric label="Ofertas" value={offers.length}/><Metric label="Pendências" value={offers.filter(item=>item.last_publication_status!=='SUCCEEDED').length}/><Metric label="Lotes parciais" value={batches.filter(item=>item.status==='PARTIAL'||item.status==='FAILED').length}/></div>{offers.length===0&&<Empty text="Nenhuma oferta persistida. O catálogo local continua sendo a fonte canônica."/>}</section><section className="rounded-3xl border border-slate-200 bg-white p-5"><h2 className="font-black">Repasses do marketplace</h2><p className="text-xs text-slate-500">Venda e liquidação financeira são fatos separados</p><div className="mt-4 grid grid-cols-3 gap-2"><Metric label="Documentos" value={settlements.length}/><Metric label="Pendentes" value={settlements.filter(item=>item.status!=='PAID').length}/><Metric label="Divergentes" value={settlements.filter(item=>item.status==='DIVERGENT').length}/></div>{settlements.length===0&&<Empty text="Nenhum documento importado. Vendas externas não aparecem conciliadas por suposição."/>}</section></div>
