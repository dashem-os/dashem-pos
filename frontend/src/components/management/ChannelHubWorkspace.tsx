...
import { AlertTriangle, CheckCircle2, Clock, Loader2, Plus, Radio, RefreshCw, Send, Tag, Wallet } from 'lucide-react'

import { usePos } from '../../context/PosContext'
import * as api from '../../services/api'
import { formatApiDateTime, formatCurrency, maskCurrencyInput, parseApiDate, parseCurrencyInput } from '../../utils/format'
...
/** What happened to each event, in the words of whoever runs the shop. RECEIVED is
 *  waiting — it must never read as an order that was processed. */
const inboxStatusLabels: Record<api.ChannelInboxEvent['status'], string> = {
  RECEIVED: 'Aguardando processamento', PROCESSING: 'Em processamento', APPLIED: 'Aplicado ao pedido',
  SUPERSEDED: 'Superado por informação mais recente', QUARANTINED: 'Em quarentena',
  NEEDS_REVIEW: 'Precisa de uma pessoa', DISCARDED: 'Descartado', EXPIRED: 'Expirado sem aplicação',
}
const resumableInbox: api.ChannelInboxEvent['status'][] = ['QUARANTINED', 'NEEDS_REVIEW']
/** The local order, named by its state. There is no order number yet, and a UUID is not a name. */
const orderStatusLabels: Record<NonNullable<api.ChannelInboxEvent['order_status']>, string> = {
  OPEN: 'Aberto no PDV', SUBMITTED: 'Enviado', CLOSED: 'Concluído', CANCELED: 'Cancelado',
}
const providerLabels: Record<string, string> = { IFOOD: 'iFood', '99FOOD': '99Food', CONTRACT_TEST: 'Canal de teste', OTHER: 'Outro canal' }
/** What a connector can do, in the words of whoever runs the shop. */
const capabilityLabels: Record<string, string> = {
  CONNECTION_VALIDATION: 'Validar a conexão', ORDER_INGRESS: 'Receber pedidos',
  ORDER_EVENTS: 'Receber mudanças e cancelamentos', ORDER_STATUS_OUTBOUND: 'Avisar o canal',
  CATALOG_PUBLICATION: 'Publicar cardápio', AVAILABILITY: 'Informar disponibilidade', SETTLEMENT_IMPORT: 'Importar repasses',
}
const orderCapabilities = ['CONNECTION_VALIDATION', 'ORDER_INGRESS', 'ORDER_EVENTS', 'ORDER_STATUS_OUTBOUND']
function ConnectionCapabilities({ connection }: { connection: api.MerchantConnection }) {
  const declared = connection.capabilities ?? []
  if (declared.length === 0) return <p className="mt-3 rounded-xl bg-state-warning-soft p-3 text-xs leading-5 text-dashem-strong">Sem conector disponível neste ambiente: esta conexão não recebe pedidos nem avisa o canal.</p>
  const shown = [...orderCapabilities, ...declared.filter((item) => !orderCapabilities.includes(item))]
  return <ul className="mt-3 space-y-1 text-xs">{shown.map((capability) => <li key={capability} className="flex items-center gap-2">{declared.includes(capability)
    ? <CheckCircle2 className="h-3.5 w-3.5 shrink-0 text-state-success" />
    : <AlertTriangle className="h-3.5 w-3.5 shrink-0 text-dashem-muted" />}<span className={declared.includes(capability) ? 'text-dashem-strong' : 'text-dashem-muted'}>{capabilityLabels[capability] ?? capability}{declared.includes(capability) ? '' : ': não disponível'}</span></li>)}</ul>
}
/** A deadline is not a removal. Until the purge exists, a passed deadline says so
 *  instead of implying the content is gone (S10.1, H20). */
function RetentionNote({ until }: { until: string }) {
  // API instants are UTC without an offset; parseApiDate reads them as such.
  const passed = (parseApiDate(until)?.getTime() ?? Infinity) <= Date.now()
  return <p className="mt-1 text-[11px] text-dashem-muted">{passed
    ? `Prazo do conteúdo venceu em ${formatApiDateTime(until)}; a limpeza ainda não é automática.`
    : `Prazo do conteúdo: ${formatApiDateTime(until)}.`}</p>
}
function InboxStatusIcon({ status }: { status: api.ChannelInboxEvent['status'] }) {
  if (status === 'APPLIED') return <CheckCircle2 className="h-3.5 w-3.5 text-state-success" />
  if (status === 'QUARANTINED' || status === 'NEEDS_REVIEW') return <AlertTriangle className="h-3.5 w-3.5 text-state-warning" />
  if (status === 'RECEIVED' || status === 'PROCESSING') return <Clock className="h-3.5 w-3.5 text-state-info" />
  return <Clock className="h-3.5 w-3.5 text-dashem-muted" />
}
...
export function ChannelHubWorkspace() {
  ...
  const resume = async (id: string) => {
    setBusy(true)
    try {
      const result = await api.resumeChannelInboxEvent(headers, id, operatorId)
      showToast(result.status === 'APPLIED' ? 'success' : 'info', result.status === 'APPLIED' ? 'Evento aplicado ao pedido.' : `O evento continua: ${inboxStatusLabels[result.status]}.`)
      await load()
    } catch (error) { showToast('error', error instanceof Error ? error.message : 'Não foi possível retomar o evento.') }
    finally { setBusy(false) }
  }
  return <section className="space-y-5 text-dashem-strong">
    <header className="flex flex-col justify-between gap-4 rounded-2xl border border-dashem-border bg-dashem-surface p-6 text-dashem-strong sm:flex-row sm:items-center"><div><p className="text-xs font-black uppercase tracking-[.16em] text-state-info">Pedidos, avisos e prazos</p><h1 className="mt-1 text-2xl font-black">Canais de venda</h1><p className="mt-2 max-w-2xl text-sm text-dashem-muted">Pedidos dos canais entram no mesmo fluxo de pedidos do PDV. Uma conexão só aparece como conectada depois que o canal a valida.</p></div><div className="flex gap-2"><button onClick={() => void load()} className="flex h-11 items-center gap-2 rounded-xl border border-dashem-border px-4 text-xs font-black"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />Atualizar</button>{permissions.includes('channel.configure') && <button onClick={() => setFormOpen(true)} className="flex h-11 items-center gap-2 rounded-xl bg-state-info px-4 text-xs font-black"><Plus className="h-4 w-4" />Nova conexão</button>}</div></header>
    <div className="grid gap-5 xl:grid-cols-[420px_1fr]"><section className="rounded-3xl border border-dashem-border bg-white p-5"><h2 className="font-black">Conexões</h2><p className="text-xs text-dashem-muted">{connections.length === 1 ? '1 conexão cadastrada' : `${connections.length} conexões cadastradas`}</p><div className="mt-4 space-y-3">{loading && <Loader2 className="h-5 w-5 animate-spin" />}{!loading && connections.length === 0 && <Empty text="Nenhum canal cadastrado. Um canal só aparece como conectado depois que ele valida a conexão." />}{connections.map((connection) => <article key={connection.id} className="rounded-2xl border border-dashem-border p-4"><div className="flex items-start justify-between"><div><p className="font-black">{providerLabels[connection.provider_code] ?? connection.provider_code}</p><p className="text-xs text-dashem-muted">Loja no canal: {connection.merchant_external_id}</p></div><span className={`rounded-full px-2 py-1 text-xs font-black ${connection.status === 'CONNECTED' ? 'bg-state-success-soft text-state-success' : 'bg-state-warning-soft text-state-warning'}`}>{connectionLabels[connection.status]}</span></div><ConnectionCapabilities connection={connection} /><p className="mt-2 text-[11px] text-dashem-muted">{connection.last_event_at ? `Último evento recebido: ${formatApiDateTime(connection.last_event_at)}` : 'Nenhum evento recebido'}{connection.last_validated_at ? ` · validada em ${formatApiDateTime(connection.last_validated_at)}` : ''}</p>{connection.last_error_code && <p className="mt-2 font-mono text-[11px] text-state-warning">{connection.last_error_code}</p>}{permissions.includes('channel.configure') && connection.status !== 'CONNECTED' && (connection.capabilities ?? []).includes('CONNECTION_VALIDATION') && <button disabled={busy} onClick={() => void validate(connection.id)} className="mt-3 h-9 w-full rounded-xl border border-state-info-border text-xs font-black text-state-info">Validar com o canal</button>}</article>)}</div></section>
      <section className="rounded-3xl border border-dashem-border bg-white p-5"><div className="flex items-center justify-between"><div><h2 className="font-black">Pedidos recebidos dos canais</h2><p className="text-xs text-dashem-muted">Cada evento do canal e o que aconteceu com ele</p></div><Radio className="h-5 w-5 text-state-info" /></div><p className="mt-3 rounded-xl border border-state-info-border bg-state-info-soft p-3 text-xs leading-5 text-dashem-strong">Sem processamento contínuo hospedado: cada evento é aplicado quando chega, por um worker quando houver, ou pelo botão Retomar. Um evento parado não avança sozinho.</p><div className="mt-4 overflow-x-auto"><ResponsiveTable className="w-full text-left text-xs"><thead><tr className="border-b text-[10px] uppercase text-dashem-muted"><th className="p-3">Recebido</th><th className="p-3">Pedido externo</th><th className="p-3">Estado</th><th className="p-3">Pedido no PDV</th></tr></thead><tbody>{events.map((event) => <tr key={event.id} className="border-b border-dashem-border"><td className="p-3">{formatApiDateTime(event.received_at)}</td><td className="p-3 font-bold">{event.external_order_id}</td><td className="p-3"><span className="flex items-center gap-1 font-black"><InboxStatusIcon status={event.status} />{inboxStatusLabels[event.status] ?? event.status}</span>{event.quarantine_reason && <p className={`mt-1 max-w-xs text-xs ${resumableInbox.includes(event.status) ? 'text-state-warning' : 'text-dashem-muted'}`}>{event.quarantine_reason}</p>}{event.retention_until ? <RetentionNote until={event.retention_until} /> : event.status === 'APPLIED' && <p className="mt-1 text-[11px] text-dashem-muted">O prazo do conteúdo começa quando o pedido terminar.</p>}<details className="mt-1 text-[11px] text-dashem-muted"><summary className="cursor-pointer">Detalhe técnico</summary><p className="mt-1 font-mono">{event.event_type}</p>{(event.last_error_code || event.quarantine_code) && <p className="font-mono">{event.last_error_code || event.quarantine_code}</p>}</details>{permissions.includes('channel.manage') && resumableInbox.includes(event.status) && <button disabled={busy} onClick={() => void resume(event.id)} className="mt-2 h-8 rounded-lg border border-state-info-border px-3 text-xs font-black text-state-info">Retomar</button>}</td><td className="p-3">{event.order_status ? <span className="font-bold">{orderStatusLabels[event.order_status] ?? event.order_status}</span> : <span className="text-dashem-muted">Nenhum pedido criado</span>}</td></tr>)}</tbody></ResponsiveTable>{!loading && events.length === 0 && <Empty text="Nenhum evento externo recebido. A tela não injeta pedidos demonstrativos." />}</div></section></div>
    <NoticesPanel headers={headers} actorId={operatorId} canManage={permissions.includes('channel.manage')} showToast={showToast} />
    <DeadlinesPanel headers={headers} showToast={showToast} />
    ...
    {formOpen && store && <ConnectionDialog storeId={store.id} actorId={operatorId} headers={headers} onClose={() => setFormOpen(false)} onCreated={async () => { setFormOpen(false); await load() }} showToast={showToast} />}
  </section>
}
...
function ChannelCatalogPanel({ headers, actorId, connections, offers, batches, mappings, loading, canManage, onChanged, showToast }: PanelProps & { offers: api.ChannelCatalogOffer[]; batches: api.ChannelPublicationBatch[]; mappings: api.ChannelCatalogMapping[] }) {
  ...
  return <section className="rounded-3xl border border-dashem-border bg-white p-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div><h2 className="font-black">Catálogo por canal</h2><p className="text-xs text-dashem-muted">Identidade canônica e publicação item a item</p></div>
    ...
    {channelOffers.length > 0 && <div className="mt-4 overflow-x-auto"><ResponsiveTable className="w-full text-left text-xs"><thead><tr className="border-b text-[10px] uppercase text-dashem-muted">{canManage && <th className="p-3 w-10"><span className="sr-only">Selecionar</span></th>}<th className="p-3">Produto</th><th className="p-3">Preço no canal</th><th className="p-3">Disponível</th><th className="p-3">Versão</th><th className="p-3">Último envio</th>{canManage && <th className="p-3 text-right">Ação</th>}</tr></thead><tbody>
      {channelOffers.map((offer) => <tr key={offer.id} className="border-b border-dashem-border">
        {canManage && <td className="p-3"><input type="checkbox" aria-label={`Selecionar ${offer.product_name ?? 'oferta'}`} checked={selected.includes(offer.id)} onChange={() => toggle(offer.id)} className="h-5 w-5 rounded border-dashem-border" /></td>}
        <td className="p-3"><p className="font-bold text-dashem-strong">{offer.product_name ?? 'Produto removido do catálogo'}</p><p className="text-[11px] text-dashem-muted">{offer.product_sku ?? '—'}</p></td>
        <td className="p-3 font-bold">{formatCurrency(offer.price)}</td>
        <td className="p-3">{offer.available ? 'Sim' : 'Não'}{offer.stock_quantity != null && <span className="block text-[11px] text-dashem-muted">estoque {offer.stock_quantity}</span>}</td>
        <td className="p-3"><span className={offer.published_version < offer.desired_version ? 'font-black text-state-warning' : ''}>{offer.published_version}/{offer.desired_version}</span></td>
...
function ProductPicker({ headers, value, onPick }: { headers: Record<string, string>; value: api.Product | null; onPick: (product: api.Product | null) => void }) {
  ...
  if (value) return <div className="rounded-xl border border-dashem-border bg-dashem-surface-elevated p-3"><p className="text-sm font-black text-dashem-strong">{value.name}</p><p className="text-[11px] text-dashem-muted">{value.sku}</p><button type="button" onClick={() => { setSearch(''); onPick(null) }} className="mt-1 text-[11px] font-black text-dashem-red">Trocar produto</button></div>
  return <div>
...
function OfferDialog({ headers, actorId, connection, offer, onClose, onSaved, showToast }: { headers: Record<string, string>; actorId: string; connection: api.MerchantConnection; offer: api.ChannelCatalogOffer | null; onClose: () => void; onSaved: () => Promise<void>; showToast: Toast }) {
  ...
  return <Modal isOpen onClose={onClose} title={offer ? 'Editar oferta do canal' : 'Nova oferta do canal'} subtitle={`${connection.provider_code} · ${connection.merchant_external_id}`}>
    <form onSubmit={submit} className="space-y-4">
      ...
      <label className="flex items-center gap-2 text-xs font-bold text-dashem-strong"><input type="checkbox" checked={available} onChange={(event) => setAvailable(event.target.checked)} className="h-5 w-5 rounded border-dashem-border" />Disponível para venda no canal</label>
      <p className="rounded-xl bg-dashem-surface-elevated p-3 text-[11px] leading-5 text-dashem-muted">Salvar altera o que se deseja publicar e devolve a oferta ao estado pendente. O preço do canal é próprio e não altera o preço da unidade.</p>
...
function MappingDialog({ headers, actorId, connection, onClose, onSaved, showToast }: { headers: Record<string, string>; actorId: string; connection: api.MerchantConnection; onClose: () => void; onSaved: () => Promise<void>; showToast: Toast }) {
  ...
  return <Modal isOpen onClose={onClose} title="Vincular código do canal" subtitle={`${connection.provider_code} · ${connection.merchant_external_id}`}>
    <form onSubmit={submit} className="space-y-4">
      ...
      <p className="rounded-xl bg-dashem-surface-elevated p-3 text-[11px] leading-5 text-dashem-muted">A identidade do Dashem continua sendo a do produto. O código do canal é a tradução usada com aquele merchant, e vale só para ele.</p>
...
function SettlementPanel({ headers, actorId, connections, settlements, loading, canManage, onChanged, showToast }: PanelProps & { settlements: api.MarketplaceSettlement[] }) {
  ...
  return <section className="rounded-3xl border border-dashem-border bg-white p-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div><h2 className="font-black">Repasses do marketplace</h2><p className="text-xs text-dashem-muted">Venda e liquidação financeira são fatos separados</p></div>
    ...
    {settlements.length > 0 && <div className="mt-4 overflow-x-auto"><ResponsiveTable className="w-full text-left text-xs"><thead><tr className="border-b text-[10px] uppercase text-dashem-muted"><th className="p-3">Documento</th><th className="p-3">Competência</th><th className="p-3">Bruto</th><th className="p-3">Descontos</th><th className="p-3">Líquido esperado</th><th className="p-3">Recebido</th><th className="p-3">Estado</th>{canManage && <th className="p-3 text-right">Ação</th>}</tr></thead><tbody>
      {settlements.map((item) => <tr key={item.id} className="border-b border-dashem-border">
        <td className="p-3"><p className="font-bold text-dashem-strong">{item.provider_document_ref}</p><p className="text-[11px] text-dashem-muted">{item.provider_code ?? '—'}{item.external_order_id ? ` · ${item.external_order_id}` : ''}</p></td>
        <td className="p-3">{formatCompetence(item.competence_date)}</td>
        <td className="p-3">{formatCurrency(item.gross_amount)}</td>
        <td className="p-3 text-[11px] text-dashem-muted">comissão {formatCurrency(item.commission_amount)}<br />taxa {formatCurrency(item.fee_amount)}<br />promoção {formatCurrency(item.promotion_amount)}<br />ajuste {formatCurrency(item.adjustment_amount)}</td>
        <td className="p-3 font-bold">{formatCurrency(item.expected_net_amount)}</td>
        <td className="p-3">{formatCurrency(item.paid_amount)}{item.payments?.length > 0 && <span className="block text-[11px] text-dashem-muted">{item.payments.length} pagamento(s)</span>}</td>
...
function SettlementDialog({ headers, actorId, connections, onClose, onSaved, showToast }: { headers: Record<string, string>; actorId: string; connections: api.MerchantConnection[]; onClose: () => void; onSaved: () => Promise<void>; showToast: Toast }) {
  ...
  return <Modal isOpen onClose={onClose} title="Importar documento de repasse" subtitle="Os valores são os que o marketplace declarou" maxWidth="lg">
    <form onSubmit={submit} className="space-y-4">
      ...
      <p className="rounded-xl bg-dashem-surface-elevated p-3 text-[11px] leading-5 text-dashem-muted">Líquido esperado por estes valores: <strong>{formatCurrency(expected)}</strong>. Quem calcula e persiste o líquido é o servidor; este número é só a conferência do que você digitou.</p>
...
function PaymentDialog({ headers, actorId, settlement, onClose, onSaved, showToast }: { headers: Record<string, string>; actorId: string; settlement: api.MarketplaceSettlement; onClose: () => void; onSaved: () => Promise<void>; showToast: Toast }) {
  ...
  return <Modal isOpen onClose={onClose} title="Registrar pagamento do repasse" subtitle={settlement.provider_document_ref}>
    <form onSubmit={submit} className="space-y-4">
      ...
      <p className="rounded-xl bg-dashem-surface-elevated p-3 text-[11px] leading-5 text-dashem-muted">A mesma referência enviada duas vezes não soma duas vezes. O pagamento acrescenta um fato ao documento e nunca reescreve o que o marketplace declarou.</p>
...
function Metric({label,value}:{label:string;value:number}){return <div className="rounded-xl bg-dashem-surface-elevated p-3"><p className="text-[10px] font-black uppercase text-dashem-muted">{label}</p><p className="mt-1 text-xl font-black">{value}</p></div>}
const noticeLabels: Record<api.ChannelNotice['status'], string> = {
  PENDING: 'Na fila', SENDING: 'Enviando', DELIVERED: 'Entregue ao canal',
  RETRY: 'Nova tentativa agendada', UNCONFIRMED: 'Sem confirmação do canal', DEAD_LETTER: 'Não entregue',
}

const noticeTypeLabels: Record<string, string> = {
  ORDER_ACCEPTED: 'Pedido aceito', ORDER_READY: 'Pedido pronto', ORDER_DISPATCHED: 'Pedido saiu',
  ORDER_CONCLUDED: 'Pedido concluído', ORDER_CANCELLED: 'Pedido cancelado',
}

/** Notices to the channel. Delivered only when the channel said so; a dead letter
 *  can be resent by whoever manages channels. Nothing here shows what was sent. */
function NoticesPanel({ headers, actorId, canManage, showToast }: { headers: Record<string, string>; actorId: string; canManage: boolean; showToast: (type: 'success' | 'error' | 'info', text: string) => void }) {
  const [notices, setNotices] = useState<api.ChannelNotice[]>([])
  const [busy, setBusy] = useState(false)
  const load = useCallback(async () => {
    if (!headers['X-Tenant-ID']) return
    try { setNotices(await api.fetchChannelNotices(headers)) }
    catch (error) { showToast('error', error instanceof Error ? error.message : 'Avisos ao canal indisponíveis.') }
  }, [headers, showToast])
  useEffect(() => { void load() }, [load])
  const resend = async (id: string) => {
    setBusy(true)
    try {
      const result = await api.resendChannelNotice(headers, id, actorId)
      showToast(result.status === 'DELIVERED' ? 'success' : 'info', result.status === 'DELIVERED' ? 'Aviso entregue ao canal.' : `O aviso segue: ${noticeLabels[result.status]}.`)
      await load()
    } catch (error) { showToast('error', error instanceof Error ? error.message : 'Não foi possível reenviar o aviso.') }
    finally { setBusy(false) }
  }
  return <section className="rounded-3xl border border-dashem-border bg-white p-5">
    <div className="flex items-center justify-between"><div><h2 className="font-black">Avisos ao canal</h2><p className="text-xs text-dashem-muted">Entregue só quando o canal confirma</p></div><Send className="h-5 w-5 text-state-info" /></div>
    <p className="mt-3 rounded-xl border border-state-info-border bg-state-info-soft p-3 text-xs leading-5 text-dashem-strong">Sem processamento contínuo hospedado: uma nova tentativa agendada só acontece quando houver outro envio, um worker ou o botão Reenviar.</p>
    <div className="mt-4 overflow-x-auto"><ResponsiveTable className="w-full text-left text-xs"><thead><tr className="border-b text-[10px] uppercase text-dashem-muted"><th className="p-3">Criado</th><th className="p-3">Pedido externo</th><th className="p-3">Aviso</th><th className="p-3">Situação</th></tr></thead><tbody>{notices.map((notice) => <tr key={notice.id} className="border-b border-dashem-border"><td className="p-3">{formatApiDateTime(notice.created_at)}</td><td className="p-3 font-bold">{notice.external_order_id ?? '—'}</td><td className="p-3">{noticeTypeLabels[notice.message_type] ?? notice.message_type}</td><td className="p-3"><span className="flex items-center gap-1 font-black">{notice.status === 'DELIVERED' ? <CheckCircle2 className="h-3.5 w-3.5 text-state-success" /> : notice.status === 'DEAD_LETTER' || notice.status === 'UNCONFIRMED' ? <AlertTriangle className="h-3.5 w-3.5 text-state-warning" /> : <Clock className="h-3.5 w-3.5 text-state-info" />}{noticeLabels[notice.status] ?? notice.status}</span>{notice.status === 'RETRY' && notice.next_retry_at && <p className="mt-1 text-[11px] text-dashem-muted">A partir de {formatApiDateTime(notice.next_retry_at)}; tentativa {notice.attempt_count}.</p>}{notice.status === 'UNCONFIRMED' && <p className="mt-1 max-w-xs text-[11px] text-dashem-muted">O canal não respondeu; ele será consultado antes de qualquer reenvio.</p>}{notice.last_error_code && notice.status !== 'DELIVERED' && <p className="mt-1 font-mono text-[11px] text-dashem-muted">{notice.last_error_code}</p>}{canManage && notice.status === 'DEAD_LETTER' && <button disabled={busy} onClick={() => void resend(notice.id)} className="mt-2 h-8 rounded-lg border border-state-info-border px-3 text-xs font-black text-state-info">Reenviar</button>}</td></tr>)}</tbody></ResponsiveTable>{notices.length === 0 && <Empty text="Nenhum aviso ao canal. Os avisos só existem quando alguém os envia; nenhum é criado sozinho." />}</div>
  </section>
}

function daysSince(value?: string | null): string {
  const at = parseApiDate(value ?? undefined)
  if (!at) return ''
  const days = Math.floor((Date.now() - at.getTime()) / 86_400_000)
  return days <= 0 ? 'hoje' : days === 1 ? 'há 1 dia' : `há ${days} dias`
}

/** Deadlines as counts: what waits for the order to end, and what passed its
 *  deadline and is still kept. A passed deadline is never shown as removed. */
function DeadlinesPanel({ headers, showToast }: { headers: Record<string, string>; showToast: (type: 'success' | 'error' | 'info', text: string) => void }) {
  const [deadlines, setDeadlines] = useState<api.ChannelDeadlines | null>(null)
  useEffect(() => {
    if (!headers['X-Tenant-ID']) return
    api.fetchChannelDeadlines(headers).then(setDeadlines)
      .catch((error) => showToast('error', error instanceof Error ? error.message : 'Prazos dos dados dos canais indisponíveis.'))
  }, [headers, showToast])
  const overdue = deadlines ? deadlines.overdue_events + deadlines.overdue_contacts : 0
  return <section className="rounded-3xl border border-dashem-border bg-white p-5">
    <div className="flex items-center justify-between"><div><h2 className="font-black">Prazos dos dados dos canais</h2><p className="text-xs text-dashem-muted">Quanto espera o fim do pedido e quanto já passou do prazo</p></div><Clock className="h-5 w-5 text-state-info" /></div>
    {deadlines && <>
      <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
        <div className="rounded-xl bg-dashem-surface-elevated p-3"><p className="text-[10px] font-black uppercase text-dashem-muted">Pedidos aguardando o fim</p><p className="mt-1 text-xl font-black">{deadlines.orders_awaiting_terminal}</p><p className="mt-1 text-[11px] leading-5 text-dashem-muted">{deadlines.orders_awaiting_terminal > 0 && deadlines.oldest_awaiting_terminal_created_at ? `O mais antigo chegou ${daysSince(deadlines.oldest_awaiting_terminal_created_at)}. O prazo do conteúdo só começa quando o pedido é concluído ou cancelado.` : 'O prazo do conteúdo só começa quando o pedido é concluído ou cancelado.'}</p></div>
        <div className={`rounded-xl p-3 ${overdue > 0 ? 'bg-state-warning-soft' : 'bg-dashem-surface-elevated'}`}><p className="text-[10px] font-black uppercase text-dashem-muted">Prazo vencido, aguardando limpeza</p><p className="mt-1 text-xl font-black">{overdue}</p><p className="mt-1 text-[11px] leading-5 text-dashem-muted">{overdue > 0 ? `${deadlines.overdue_events} ${deadlines.overdue_events === 1 ? 'evento' : 'eventos'} e ${deadlines.overdue_contacts} ${deadlines.overdue_contacts === 1 ? 'contato' : 'contatos'}; o primeiro venceu em ${formatApiDateTime(deadlines.oldest_overdue_until ?? '')}. O conteúdo continua guardado.` : 'Nenhum registro com prazo vencido.'}</p></div>
      </div>
      <p className="mt-3 rounded-xl border border-state-info-border bg-state-info-soft p-3 text-xs leading-5 text-dashem-strong">{deadlines.last_cleanup_at ? `Última limpeza: ${formatApiDateTime(deadlines.last_cleanup_at)}.` : 'Nenhuma limpeza foi executada: a limpeza ainda não existe.'} Sem processamento contínuo hospedado, ela também não seria automática. Prazo vencido não quer dizer dado removido.</p>
    </>}
  </section>
}

function ConnectionDialog({ storeId, actorId, headers, onClose, onCreated, showToast }: { storeId: string; actorId: string; headers: Record<string, string>; onClose: () => void; onCreated: () => Promise<void>; showToast: (type: 'success' | 'error' | 'info', text: string) => void }) {
  const [form, setForm] = useState({ provider: 'IFOOD', merchant: '', name: '' }); const [saving, setSaving] = useState(false)
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setSaving(true); try { const result = await api.createMerchantConnection(headers, crypto.randomUUID(), { store_id: storeId, provider_code: form.provider, merchant_external_id: form.merchant, channel_name: form.name, actor_id: actorId }); showToast('success', 'Conexão cadastrada como não validada.'); if (result.connection) await onCreated() } catch (error) { showToast('error', error instanceof Error ? error.message : 'Não foi possível cadastrar a conexão.') } finally { setSaving(false) } }
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-dashem-strong/70 p-4"><form onSubmit={submit} className="responsive-dialog w-full max-w-lg space-y-3 rounded-3xl bg-white p-6"><div className="flex justify-between"><div><p className="text-xs font-black uppercase text-state-info">Cadastro de canal</p><h2 className="text-xl font-black">Nova conexão</h2></div><button type="button" onClick={onClose}>×</button></div><label className="block text-xs font-black">Canal<select value={form.provider} onChange={(e) => setForm({ ...form, provider: e.target.value })} className="mt-1 h-11 w-full rounded-xl border px-3"><option value="IFOOD">iFood</option><option value="99FOOD">99Food</option><option value="OTHER">Outro canal</option></select></label><Field label="Identificador da loja no canal" value={form.merchant} onChange={(value) => setForm({ ...form, merchant: value })} /><Field label="Nome do canal" value={form.name} onChange={(value) => setForm({ ...form, name: value })} /><p className="text-xs leading-5 text-dashem-muted">Cadastrar não significa conectar: a conexão só muda depois que o canal a valida. Nenhuma credencial é digitada aqui.</p><button disabled={saving} className="h-11 w-full rounded-xl bg-state-info text-sm font-black text-white">{saving ? 'Salvando...' : 'Cadastrar, ainda não conectada'}</button></form></div>
}
