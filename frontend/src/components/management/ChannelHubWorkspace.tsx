...
export function ChannelHubWorkspace() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setConnections(nextConnections); setEvents(nextEvents); setOffers(catalog.offers); setBatches(catalog.batches); setSettlements(nextSettlements)
  ...
  return <section className="space-y-5 text-slate-950">
    <header className="flex flex-col justify-between gap-4 rounded-3xl bg-[#07172b] p-6 text-dashem-strong sm:flex-row sm:items-center"><div><p className="text-xs font-black uppercase tracking-[.16em] text-cyan-400">Omnichannel · inbox durável</p><h1 className="mt-1 text-2xl font-black">Dashem Channel Hub</h1><p className="mt-2 max-w-2xl text-sm text-dashem-muted">Eventos externos entram no mesmo Order Engine. Conexão só aparece ativa após validação real do adapter.</p></div><div className="flex gap-2"><button onClick={() => void load()} className="flex h-11 items-center gap-2 rounded-xl border border-dashem-border px-4 text-xs font-black"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />Atualizar</button>{permissions.includes('channel.configure') && <button onClick={() => setFormOpen(true)} className="flex h-11 items-center gap-2 rounded-xl bg-cyan-600 px-4 text-xs font-black"><Plus className="h-4 w-4" />Nova conexão</button>}</div></header>
    ...
    <div className="grid gap-5 lg:grid-cols-2"><section className="rounded-3xl border border-slate-200 bg-white p-5"><h2 className="font-black">Catálogo por canal</h2><p className="text-xs text-slate-500">Identidade canônica e publicação item a item</p><div className="mt-4 grid grid-cols-3 gap-2"><Metric label="Ofertas" value={offers.length}/><Metric label="Pendências" value={offers.filter(item=>item.last_publication_status!=='SUCCEEDED').length}/><Metric label="Lotes parciais" value={batches.filter(item=>item.status==='PARTIAL'||item.status==='FAILED').length}/></div>{offers.length===0&&<Empty text="Nenhuma oferta persistida. O catálogo local continua sendo a fonte canônica."/>}</section><section className="rounded-3xl border border-slate-200 bg-white p-5"><h2 className="font-black">Repasses do marketplace</h2><p className="text-xs text-slate-500">Venda e liquidação financeira são fatos separados</p><div className="mt-4 grid grid-cols-3 gap-2"><Metric label="Documentos" value={settlements.length}/><Metric label="Pendentes" value={settlements.filter(item=>item.status!=='PAID').length}/><Metric label="Divergentes" value={settlements.filter(item=>item.status==='DIVERGENT').length}/></div>{settlements.length===0&&<Empty text="Nenhum documento importado. Vendas externas não aparecem conciliadas por suposição."/>}</section></div>
