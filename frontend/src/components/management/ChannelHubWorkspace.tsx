...
export function ChannelHubWorkspace() {
  ...
  return <section className="space-y-5 text-slate-950">
    ...
    <div className="grid gap-5 lg:grid-cols-2"><section className="rounded-3xl border border-slate-200 bg-white p-5"><h2 className="font-black">Catálogo por canal</h2><p className="text-xs text-slate-500">Identidade canônica e publicação item a item</p><div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3"><Metric label="Ofertas" value={offers.length}/><Metric label="Pendências" value={offers.filter(item=>item.last_publication_status!=='SUCCEEDED').length}/><Metric label="Lotes parciais" value={batches.filter(item=>item.status==='PARTIAL'||item.status==='FAILED').length}/></div>{offers.length===0&&<Empty text="Nenhuma oferta persistida. O catálogo local continua sendo a fonte canônica."/>}</section><section className="rounded-3xl border border-slate-200 bg-white p-5"><h2 className="font-black">Repasses do marketplace</h2><p className="text-xs text-slate-500">Venda e liquidação financeira são fatos separados</p><div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-2"><Metric label="Documentos" value={settlements.length}/><Metric label="Pendentes" value={settlements.filter(item=>item.status!=='PAID').length}/><Metric label="Divergentes" value={settlements.filter(item=>item.status==='DIVERGENT').length}/></div>{settlements.length===0&&<Empty text="Nenhum documento importado. Vendas externas não aparecem conciliadas por suposição."/>}</section></div>
