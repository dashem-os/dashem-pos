...
export function DeviceManager() {
  ...
  const load = async () => {
    ...
    setLoading(true)
    try {
      const [nextDevices, nextRegisters, nextPoints, nextRules] = await Promise.all([
      ...
      setDevices(nextDevices); setRegisters(nextRegisters); setPoints(nextPoints); setRules(nextRules)
  ...
  return <div className="space-y-6">
    <section className="rounded-3xl border border-dashem-border bg-dashem-surface p-6">
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="text-[11px] font-black uppercase tracking-[.18em] text-sky-400">Infraestrutura da unidade</p><h1 className="mt-2 text-3xl font-black text-white">Terminais, produção e impressão</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-dashem-muted">Cadastre o ponto físico, vincule-o ao caixa ou à produção e acompanhe autorização e presença sem expor segredos.</p></div><div className="flex gap-2"><button onClick={() => void load()} className="flex h-11 items-center gap-2 rounded-xl border border-dashem-border px-4 text-xs font-black"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />Atualizar</button>{canConfigure && <button onClick={() => setDialog('DEVICE')} className="h-11 rounded-xl bg-dashem-red px-5 text-xs font-black text-white">Novo dispositivo</button>}</div></div>
