...
export function OperationalSessionGate({
  ...
  if (failure) {
    ...
    return <main className="flex min-h-screen items-center justify-center bg-[#06101f] p-6"><section className="w-full max-w-lg rounded-3xl bg-white p-8 text-center text-slate-900 shadow-2xl"><ShieldAlert className="mx-auto h-8 w-8 text-amber-700" /><h1 className="mt-4 text-xl font-black text-slate-950">{ended ? 'Sessão operacional encerrada' : 'Validação indisponível'}</h1><p role="alert" className="mt-3 text-sm leading-6 text-slate-700">{failure.message}</p><button onClick={() => ended ? navigateTo('/operate') : setAttempt(value => value + 1)} className="mt-6 h-12 w-full rounded-xl bg-slate-950 font-black text-white focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-ink">{ended ? 'Voltar à identificação' : 'Tentar novamente'}</button></section></main>
  }
