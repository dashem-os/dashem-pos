...
const integer = (value: string) => value.replace(/\D/g, '').replace(/^0+(?=\d)/, '')
...
export function ServicePlansView({ onOrganizations }: { onOrganizations?: () => void }) {
  ...
  const startEdit = (plan: ServicePlan) => {
    ...
    setForm({
      ...
      devices: plan.terminal_limit?.toString() || '', storage: plan.storage_limit_mib?.toString() || '',
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const input: ServicePlanInput = {
      ...
      terminal_limit: optionalPositive(form.devices), storage_limit_mib: optionalPositive(form.storage),
    ...
    try {
      ...
      setNotice(editingId === 'new' ? 'Plano comercial criado e disponível para contratação.' : 'Nova versão comercial registrada e auditada.')
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'Não foi possível salvar o plano comercial.')
    } finally { setSaving(false) }
  }

  return <div className="mx-auto max-w-[1500px] p-5 sm:p-8">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div><p className="text-xs font-black uppercase tracking-wider text-[#E12120]">Catálogo comercial</p><h2 className="mt-2 text-3xl font-black">Planos comerciais</h2><p className="mt-2 max-w-3xl text-slate-500">Defina a oferta, o preço-base, as capabilities e os tetos. Descontos pertencem ao contrato do tenant e preservam o valor de tabela.</p></div>
      <button onClick={startNew} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-[#E12120] px-5 text-sm font-black text-white"><Plus className="h-4 w-4" />Novo plano</button>
    </div>
    {error && <p className="mt-5 rounded-xl border border-[#ffbf00] bg-amber-50 p-4 text-sm font-bold text-[#6b4b00]">{error}</p>}
    {notice && <div className="mt-5 flex flex-col gap-3 rounded-xl border border-emerald-300 bg-emerald-50 p-4 text-sm font-bold text-emerald-800 sm:flex-row sm:items-center sm:justify-between"><p className="flex items-center gap-2"><CheckCircle2 className="h-5 w-5" />{notice}</p>{onOrganizations && <button type="button" onClick={onOrganizations} className="h-10 rounded-xl border border-emerald-300 bg-white px-4 text-sm font-black text-emerald-800">Vincular a uma organização</button>}</div>}

    {editingId && <form onSubmit={submit} className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex items-start justify-between"><div><h3 className="text-xl font-black">{editingId === 'new' ? 'Cadastrar plano' : 'Editar plano'}</h3><p className="mt-1 text-sm text-slate-500">Cada salvamento cria uma nova versão auditada do preço, limites e capabilities.</p></div><button type="button" onClick={() => setEditingId(null)} className="rounded-xl border border-slate-200 p-2"><X className="h-5 w-5" /></button></div>
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Field label="Código" value={form.code} error={fieldErrors.code} onChange={value => set('code', value.toUpperCase().replace(/[^A-Z0-9_-]/g, ''))} />
        <Field label="Nome do plano" value={form.name} error={fieldErrors.name} onChange={value => set('name', value)} />
        <Field label="Preço-base mensal (R$)" value={form.monthlyPrice} error={fieldErrors.monthlyPrice} inputMode="decimal" onChange={value => set('monthlyPrice', value.replace(/[^\d,.]/g, ''))} />
        <label className="text-sm font-black">Situação<button type="button" disabled={editingId === 'new'} onClick={() => set('isActive', !form.isActive)} className={`mt-2 flex h-11 w-full items-center justify-center rounded-xl border font-black disabled:cursor-not-allowed ${form.isActive ? 'border-emerald-300 bg-emerald-50 text-emerald-700' : 'border-slate-300 bg-slate-100 text-slate-500'}`}>{editingId === 'new' ? 'Ativo ao cadastrar' : form.isActive ? 'Ativo' : 'Inativo'}</button></label>
        <div className="md:col-span-2 xl:col-span-4"><Field label="Descrição comercial" value={form.description} onChange={value => set('description', value)} /></div>
        <Field label="Unidades" value={form.units} error={fieldErrors.units} inputMode="numeric" placeholder="Sem limite" onChange={value => set('units', integer(value))} />
        ...
        <Field label="Armazenamento (MiB)" value={form.storage} error={fieldErrors.storage} inputMode="numeric" placeholder="Sem limite" onChange={value => set('storage', integer(value))} />
...
function Field({ label, value, onChange, error, inputMode, placeholder }: { label: string; value: string; onChange: (value: string) => void; error?: string; inputMode?: React.HTMLAttributes<HTMLInputElement>['inputMode']; placeholder?: string }) {
  const displayLabel = label === 'Armazenamento (MiB)' ? 'Armazenamento (MiB) — teto da oferta' : label
