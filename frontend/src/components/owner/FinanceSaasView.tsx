...
type FinanceTab = 'billing' | 'receipts' | 'contracts' | 'projection'
...
const invoiceLabel: Record<SaasInvoiceStatus, string> = {
  DRAFT: 'Rascunho', NO_PAYMENT_DUE: 'Sem pagamento devido', OPEN: 'Aberta', PARTIALLY_PAID: 'Parcialmente paga', PAID: 'Paga',
  OVERDUE: 'Vencida', VOID: 'Anulada', UNCOLLECTIBLE: 'Incobrável',
}

export function FinanceSaasView({ onTenant, onOrganizations, onPlans }: { onTenant: (tenantId: string) => void; onOrganizations: () => void; onPlans: () => void }) {
  ...
  const openTab = (tab: FinanceTab, target?: string) => {
    setActiveTab(tab)
  ...
  const hasSubscriptions = (overview?.subscriptions.length ?? 0) > 0
  ...
  const contractCards: Array<{ label: string; value: string | number; filter: ContractFilter; hint: string }> = [
    { label: 'MRR líquido', value: money(overview?.contracted_mrr ?? 0), filter: 'ACTIVE', hint: 'Valor recorrente após descontos' },
    { label: 'MRR bruto', value: money(overview?.gross_mrr ?? 0), filter: 'ACTIVE', hint: 'Valor recorrente de tabela' },
    { label: 'Descontos ativos', value: money(overview?.discount_mrr ?? 0), filter: 'ACTIVE', hint: 'Bruto menos líquido' },
    { label: 'Assinaturas ativas', value: overview?.active_subscriptions ?? 0, filter: 'ACTIVE', hint: 'Inclui contratos com desconto de 100%' },
  ]
  ...
  return <div className="mx-auto max-w-[1500px] p-5 sm:p-8">
    ...
    {loading ? <Loader2 className="mx-auto my-24 h-8 w-8 animate-spin text-[#E12120]" /> : <>
      {!hasSubscriptions && <FinanceSetupCallout onOrganizations={onOrganizations} onPlans={onPlans} />}
      <nav aria-label="Áreas do Financeiro SaaS" className="mt-6 flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-sm">{([['billing', 'Faturamento SaaS'], ['receipts', 'Recebimentos e Cobranças'], ['contracts', 'Base Contratual'], ['projection', 'Projeção Financeira Reconstruível']] as Array<[FinanceTab, string]>).map(([key, label]) => <button type="button" key={key} onClick={() => openTab(key)} className={`shrink-0 rounded-xl px-4 py-3 text-sm font-black transition ${activeTab === key ? 'bg-[#022444] text-white' : 'text-slate-500 hover:bg-slate-100'}`}>{label}</button>)}</nav>
      {activeTab === 'billing' && <>
        <SectionTitle title="Faturamento SaaS" detail="Valores derivados das faturas persistidas." />
        <section className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{invoiceCards.map(card => <MetricCard key={card.label} {...card} active={invoiceFilter === card.filter} onClick={() => { setInvoiceFilter(card.filter); setScrollTarget('saas-invoices') }} />)}</section>
        <section className="mt-5 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-end sm:justify-between"><label className="text-sm font-black">Competência<input aria-label="Competência" type="month" value={competence} onChange={event => setCompetence(event.target.value)} className="mt-2 block h-11 rounded-xl border border-slate-300 px-3 font-medium" /></label><div className="flex flex-wrap gap-2"><button onClick={async () => { setWorking(true); setError(''); try { const changed = await markSaasInvoicesOverdue(new Date().toISOString().slice(0, 10)); setNotice(`${changed.length} fatura(s) marcada(s) como vencida(s) a partir de fatos reais.`); await Promise.all([loadOverview(), loadInvoices(), loadPayments()]) } catch (cause) { setError(cause instanceof Error ? cause.message : 'Não foi possível derivar vencimentos.') } finally { setWorking(false) } }} disabled={working || !hasSubscriptions} className="flex h-11 items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 text-sm font-black disabled:opacity-50"><CalendarClock className="h-4 w-4" />Derivar vencimentos</button><button onClick={() => void generate()} disabled={working || !competence || !hasSubscriptions} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-[#E12120] px-5 text-sm font-black text-white disabled:opacity-50">{working ? <Loader2 className="h-4 w-4 animate-spin" /> : <FilePlus2 className="h-4 w-4" />}Gerar faturas da competência</button></div></section>
      ...
      {activeTab === 'receipts' && <>
        <SectionTitle title="Recebimentos e Cobranças" detail="Valores líquidos derivados de pagamentos, alocações, estornos e vencimentos persistidos." />
        <section className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{paymentCards.map(card => <MetricCard key={card.label} label={card.label} value={card.value} hint={card.hint} active={card.target === 'INVOICES' && invoiceFilter === card.filter} onClick={() => { if (card.target === 'INVOICES' && card.filter) { setInvoiceFilter(card.filter); openTab('billing', 'saas-invoices') } else setScrollTarget('saas-payments') }} />)}</section>
        <PaymentTable rows={payments} total={paymentTotal} onRefund={openRefund} />
      </>}
      {activeTab === 'contracts' && <>
        <SectionTitle title="Base Contratual" detail="Assinaturas persistidas que alimentam a geração das faturas." />
        ...
        <SubscriptionTable rows={contractRows} filter={contractFilter} onAll={() => setContractFilter('ALL')} onTenant={onTenant} hasSubscriptions={hasSubscriptions} onOrganizations={onOrganizations} />
      </>}
      {activeTab === 'projection' && <ProjectionSection projection={projection} history={projectionHistory} filter={projectionFilter} onFilter={setProjectionFilter} onRebuild={rebuildProjection} onTenant={onTenant} working={working} hasSubscriptions={hasSubscriptions} onOrganizations={onOrganizations} onInvoices={() => { setInvoiceFilter('OVERDUE'); openTab('billing', 'saas-invoices') }} onPayments={() => openTab('receipts', 'saas-payments')} />}
    </>}
...
function FinanceSetupCallout({ onOrganizations, onPlans }: { onOrganizations: () => void; onPlans: () => void }) {
  return <section className="mt-6 rounded-2xl border border-amber-300 bg-amber-50 p-5"><p className="text-xs font-black uppercase tracking-wider text-amber-800">Configuração comercial necessária</p><h3 className="mt-2 text-xl font-black">Ainda não existe assinatura SaaS persistida</h3><p className="mt-2 max-w-3xl text-sm text-slate-600">Cadastre um plano comercial e salve a primeira versão do contrato de uma organização. Somente depois disso o Financeiro poderá gerar faturas e materializar MRR/ARR.</p><div className="mt-4 flex flex-wrap gap-2"><button onClick={onPlans} className="h-10 rounded-xl bg-[#022444] px-4 text-sm font-black text-white">Cadastrar plano</button><button onClick={onOrganizations} className="h-10 rounded-xl border border-amber-300 bg-white px-4 text-sm font-black">Regularizar organizações</button></div></section>
}
...
function PaymentTable({ rows, total, onRefund }: { rows: SaasPaymentListItem[]; total: number; onRefund: (row: SaasPaymentListItem) => void }) {
  return <section id="saas-payments" className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 p-5"><h3 className="font-black">Recebimentos SaaS</h3><p className="text-sm text-slate-500">{total} fato(s) persistido(s); nenhum pagamento do tenant aparece aqui.</p></div><div className="overflow-x-auto"><ResponsiveTable className="w-full min-w-[1000px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-400"><tr><th className="px-5 py-3">Cliente</th><th className="px-5 py-3">Data</th><th className="px-5 py-3">Origem</th><th className="px-5 py-3">Situação</th><th className="px-5 py-3">Valor</th><th className="px-5 py-3">Alocado / estornado</th><th className="px-5 py-3">Ação</th></tr></thead><tbody className="divide-y divide-slate-100">{rows.map(row => <tr key={row.payment.id}><td className="px-5 py-4 font-black">{row.tenant_name}</td><td className="px-5 py-4">{formatProductDateTime(row.payment.received_at)}</td><td className="px-5 py-4">{row.payment.provider}<p className="mt-1 text-xs text-slate-400">{row.payment.payment_method_summary || 'Meio sanitizado não informado'}</p></td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-black ${paymentBadge[row.payment.status]}`}>{paymentLabel[row.payment.status]}</span></td><td className="px-5 py-4 font-black">{money(row.payment.amount)}</td><td className="px-5 py-4">{money(row.allocated_amount)}<p className="mt-1 text-xs text-slate-500">Estornado {money(row.refunded_amount)}</p></td><td className="px-5 py-4">{Number(row.allocated_amount) > Number(row.refunded_amount) && ['SUCCEEDED', 'PARTIALLY_REFUNDED'].includes(row.payment.status) ? <button onClick={() => onRefund(row)} className="flex items-center gap-1 rounded-lg border border-red-200 px-3 py-2 text-xs font-black text-red-700"><RotateCcw className="h-3.5 w-3.5" />Estornar</button> : '—'}</td></tr>)}{rows.length === 0 && <tr><td colSpan={7} className="px-5 py-12 text-center text-slate-500">Nenhum recebimento persistido.</td></tr>}</tbody></ResponsiveTable></div></section>
}

function SubscriptionTable({ rows, filter, onAll, onTenant, hasSubscriptions, onOrganizations }: { rows: PlatformFinanceSubscription[]; filter: ContractFilter; onAll: () => void; onTenant: (tenantId: string) => void; hasSubscriptions: boolean; onOrganizations: () => void }) {
  return <section className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-slate-100 p-5"><div><h3 className="font-black">Contratos SaaS</h3><p className="text-sm text-slate-500">{filter === 'ALL' ? 'Todos os registros contratuais' : `Filtro ativo: ${filter}`}</p></div>{filter !== 'ALL' && <button onClick={onAll} className="text-sm font-black text-[#E12120]">Ver todos</button>}</div><div className="overflow-x-auto"><ResponsiveTable className="w-full min-w-[900px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-400"><tr><th className="px-5 py-3">Cliente</th><th className="px-5 py-3">Plano</th><th className="px-5 py-3">Assinatura</th><th className="px-5 py-3">Mensalidade</th><th className="px-5 py-3">Conta de cobrança</th><th className="px-5 py-3">Regra de vencimento</th></tr></thead><tbody className="divide-y divide-slate-100">{rows.map(item => <tr key={item.tenant_id}><td className="px-5 py-4"><button onClick={() => onTenant(item.tenant_id)} className="flex items-center gap-2 text-left font-black hover:text-[#E12120]">{item.tenant_name}<ArrowRight className="h-4 w-4" /></button><p className="mt-1 text-xs text-slate-400">Contrato v{item.contract_version ?? '—'}</p></td><td className="px-5 py-4">{item.plan_name || 'Sem plano'}</td><td className="px-5 py-4">{subscriptionLabel[item.subscription_status]}</td><td className="px-5 py-4 font-bold">{money(item.monthly_amount)}<p className="mt-1 text-xs font-medium text-slate-500">Bruto {money(item.gross_monthly_amount)} · desconto {money(item.discount_amount)}</p></td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-black ${item.billing_account_ready ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-800'}`}>{item.billing_account_ready ? 'APTA' : 'INCOMPLETA'}</span><p className="mt-2 text-xs text-slate-500">{item.billing_contact_email || 'Contato não configurado'}</p></td><td className="px-5 py-4 font-bold">{item.billing_day == null ? <span className="text-amber-700">Não informado</span> : `Todo dia ${item.billing_day}`}</td></tr>)}{rows.length === 0 && <tr><td colSpan={6} className="px-5 py-12 text-center text-slate-500">{hasSubscriptions ? 'Nenhum contrato corresponde ao filtro.' : <div><p className="font-bold text-slate-700">Nenhuma assinatura SaaS foi criada.</p><button onClick={onOrganizations} className="mt-3 rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-black text-[#022444]">Abrir organizações</button></div>}</td></tr>}</tbody></ResponsiveTable></div></section>
}
...
function ProjectionSection({ projection, history, filter, onFilter, onRebuild, onTenant, working, hasSubscriptions, onOrganizations, onInvoices, onPayments }: { projection: SaasFinanceProjectionDetail | null; history: SaasFinanceDailyMetric[]; filter: 'ALL' | 'INCLUDED' | SaasMrrMovementType; onFilter: (value: 'ALL' | 'INCLUDED' | SaasMrrMovementType) => void; onRebuild: () => Promise<void>; onTenant: (tenantId: string) => void; working: boolean; hasSubscriptions: boolean; onOrganizations: () => void; onInvoices: () => void; onPayments: () => void }) {
  if (!projection && !hasSubscriptions) return <section className="mt-7"><SectionTitle title="Projeção financeira reconstruível" detail="A projeção depende de pelo menos uma assinatura SaaS persistida." /><article className="mt-4 rounded-2xl border border-dashed border-amber-300 bg-amber-50 p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><BarChart3 className="h-5 w-5 text-amber-700" /><span className="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-black text-amber-800">Aguardando base contratual</span></div><h4 className="mt-4 font-black">Não há fatos para materializar</h4><p className="mt-2 text-sm text-slate-600">Salve o primeiro contrato com plano, mensalidade e situação da assinatura antes de reconstruir MRR e ARR.</p></div><button onClick={onOrganizations} className="h-11 rounded-xl bg-[#022444] px-5 text-sm font-black text-white">Regularizar contratos</button></div></article></section>
  ...
  const movementCards: Array<{ label: string; value: string; filter: 'ALL' | 'INCLUDED' | SaasMrrMovementType; hint: string }> = [
    { label: 'MRR líquido', value: money(metric.contracted_mrr), filter: 'INCLUDED', hint: 'Receita recorrente após descontos' },
    { label: 'MRR bruto', value: money(metric.gross_mrr), filter: 'INCLUDED', hint: 'Valor recorrente antes dos descontos' },
    { label: 'Descontos', value: money(metric.discount_mrr), filter: 'INCLUDED', hint: 'Redução comercial vigente' },
    { label: 'ARR projetado', value: money(metric.projected_arr), filter: 'INCLUDED', hint: 'MRR líquido × 12; não é caixa' },
