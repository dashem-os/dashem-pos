...
function OrganizationsView({ overview, filter, onFilter, onTenant }: { overview: PlatformOverview | null; filter: OrganizationFilter; onFilter: (filter: OrganizationFilter) => void; onTenant: (tenant: PlatformTenantSummary) => void }) {
  ...
  return <div className="mx-auto max-w-[1500px] p-5 sm:p-8">
    ...
    <section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">{!overview ? <Loader2 className="mx-auto my-20 h-7 w-7 animate-spin text-[#E12120]" /> : <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left"><thead className="bg-slate-50 text-xs font-black uppercase tracking-wider text-slate-400"><tr><th className="p-4">Cliente</th><th className="p-4">Classificação</th><th className="p-4">Ciclo</th><th className="p-4">Cadastro</th><th className="p-4" /></tr></thead><tbody className="divide-y divide-slate-100">{tenants.map(tenant => <tr key={tenant.id} className="hover:bg-slate-50"><td className="p-4"><button onClick={() => onTenant(tenant)} className="text-left"><p className="font-black">{tenant.name}</p><p className="text-sm text-slate-500">{tenant.legal_name || tenant.slug}</p><p className="mt-1 font-mono text-xs text-slate-400">{tenant.tax_id || 'CPF/CNPJ não informado'}</p></button></td><td className="p-4 text-sm font-black">{typeLabel[tenant.customer_type ?? 'TEST']}</td><td className="p-4"><span className={`rounded-full px-2.5 py-1 text-xs font-black ${tenant.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{statusLabel[tenant.status]}</span></td><td className="p-4"><span className={`text-xs font-black ${tenant.profile_complete ? 'text-emerald-700' : 'text-amber-700'}`}>{tenant.profile_complete ? 'COMPLETO' : 'INCOMPLETO'}</span></td><td className="p-4"><button onClick={() => onTenant(tenant)} className="flex items-center gap-2 rounded-xl bg-[#022444] px-3 py-2 text-xs font-black text-white">Abrir <ArrowRight className="h-4 w-4" /></button></td></tr>)}</tbody></table>{tenants.length === 0 && <p className="py-16 text-center text-sm font-semibold text-slate-500">Nenhum cliente corresponde ao filtro.</p>}</div>}</section>
  </div>
}
