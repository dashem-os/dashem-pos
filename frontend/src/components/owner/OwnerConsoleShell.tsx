...
export function OwnerConsoleShell({ me }: { me: AuthMe }) {
  ...
  return <div className="min-h-screen bg-[#f4f6f9] text-[#022444] lg:grid lg:h-dvh lg:grid-cols-[260px_minmax(0,1fr)] lg:overflow-hidden">
    <aside className={`${mobileNav ? 'flex' : 'hidden'} fixed inset-y-0 left-0 z-40 w-[min(90vw,280px)] flex-col overflow-y-auto bg-[#022444] p-5 text-white shadow-2xl lg:static lg:flex lg:h-dvh lg:w-auto lg:shadow-none`}>
      <div className="flex items-center justify-between"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#E12120] font-black shadow-lg shadow-red-900/30">D</div><div><p className="font-black">DASHEM</p><p className="text-[10px] font-bold uppercase tracking-[.2em] text-slate-400">Control</p></div></div><button className="rounded-xl border border-white/10 p-2 lg:hidden" aria-label="Fechar menu" onClick={() => setMobileNav(false)}><X className="h-5 w-5" /></button></div>
      <nav className="my-6 shrink-0 space-y-2">
    ...
    {mobileNav && <button className="fixed inset-0 z-30 bg-[#022444]/50 lg:hidden" aria-label="Fechar menu" onClick={() => setMobileNav(false)} />}

    <main className="min-w-0 lg:h-dvh lg:overflow-y-auto"><header className="sticky top-0 z-20 flex min-h-20 flex-wrap items-center justify-between gap-3 py-3 border-b border-slate-200 bg-white px-5 sm:px-8"><div className="flex items-center gap-3"><button className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-slate-200 lg:hidden" aria-label="Abrir menu" onClick={() => setMobileNav(true)}><Menu className="h-5 w-5" /></button><div><p className="text-xs font-bold uppercase tracking-[.16em] text-slate-400">Dashem Control</p><h1 className="text-lg font-black">{title}</h1></div></div>{view === 'organizations' && <button aria-label="Novo cliente" disabled={checkingPlans} onClick={openCreateTenant} className="flex h-11 items-center gap-2 rounded-xl bg-[#E12120] px-4 text-sm font-black text-white shadow-lg shadow-red-900/20 disabled:opacity-50">{checkingPlans ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}<span className="hidden sm:inline">{checkingPlans ? 'Verificando…' : 'Novo cliente'}</span></button>}</header>
...
function OverviewView({ overview, health, onOrganizations, onHealth, onTenant }: { overview: PlatformOverview | null; health: PlatformSystemHealth | null; onOrganizations: (filter: OrganizationFilter) => void; onHealth: () => void; onTenant: (tenant: PlatformTenantSummary) => void }) {
  ...
  return <div className="mx-auto max-w-[1500px] p-5 sm:p-8"><section className="rounded-3xl bg-[#022444] p-7 text-white sm:p-9"><div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[.18em] text-[#E12120]">Control plane · dados em tempo real</p><h2 className="mt-3 text-3xl font-black">Operação da plataforma</h2><p className="mt-3 max-w-2xl leading-7 text-slate-300">Clientes, contratos, onboarding e saúde técnica conectados aos registros reais do Dashem.</p></div><button onClick={onHealth} className={`min-w-60 rounded-2xl border p-4 text-left ${health?.status === 'HEALTHY' ? 'border-emerald-400/20 bg-emerald-400/10' : 'border-amber-400/20 bg-amber-400/10'}`}><p className="text-xs font-black uppercase text-slate-400">Saúde da plataforma</p><p className="mt-2 flex items-center gap-2 font-black">{health ? statusText(health.status) : 'Verificando…'}<ArrowRight className="h-4 w-4" /></p></button></div></section><section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{cards.map(card => <button key={card.label} onClick={() => onOrganizations(card.filter)} className="rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#E12120]/40 hover:shadow-md"><div className="flex items-start justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-slate-400">{card.label}</p><p className="mt-3 text-3xl font-black">{overview ? card.value : '—'}</p></div><div className="rounded-xl bg-slate-100 p-2.5"><card.icon className="h-5 w-5" /></div></div><p className="mt-4 flex items-center justify-between text-xs font-semibold text-slate-500">{card.hint}<ArrowRight className="h-4 w-4" /></p></button>)}</section><section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><div><h3 className="font-black">Clientes recentes</h3><p className="text-sm text-slate-500">Abra o workspace contratual do cliente.</p></div><button onClick={() => onOrganizations('ALL')} className="text-sm font-black text-[#E12120]">Ver todos</button></div><div className="mt-5 divide-y divide-slate-100">{overview?.tenants.slice(0, 5).map(tenant => <button key={tenant.id} onClick={() => onTenant(tenant)} className="flex w-full items-center justify-between py-4 text-left"><div><p className="font-black">{tenant.name}</p><p className="text-sm text-slate-500">{tenant.legal_name || tenant.slug}</p></div><div className="flex items-center gap-3"><span className="text-xs font-black text-slate-500">{statusLabel[tenant.status]}</span><ArrowRight className="h-4 w-4" /></div></button>)}{overview?.tenants.length === 0 && <p className="py-10 text-center text-sm font-semibold text-slate-500">Nenhum cliente cadastrado.</p>}</div></section></div>
}
...
function OrganizationsView({ overview, filter, onFilter, onTenant }: { overview: PlatformOverview | null; filter: OrganizationFilter; onFilter: (filter: OrganizationFilter) => void; onTenant: (tenant: PlatformTenantSummary) => void }) {
  ...
  return <div className="mx-auto max-w-[1500px] p-5 sm:p-8">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div><p className="text-xs font-black uppercase tracking-wider text-[#E12120]">Relação comercial</p><h2 className="mt-2 text-3xl font-black">Organizações</h2><p className="mt-2 text-slate-500">Cadastro contratual, ciclo de vida, plano e capabilities.</p></div>
      <label className="relative block lg:w-96"><Search className="absolute left-4 top-3.5 h-4 w-4 text-slate-400" /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Nome, razão social, CPF, CNPJ ou identificador" className="h-11 w-full rounded-xl border border-slate-300 bg-white pl-11 pr-4 font-semibold" /></label>
    </div>
    <div className="mt-6 flex flex-wrap gap-2">{([['ALL', 'Todos'], ['ACTIVE', 'Em operação'], ['IMPLEMENTATION', 'Em implantação'], ['ATTENTION', 'Requer atenção']] as Array<[OrganizationFilter, string]>).map(([key, label]) => <button key={key} onClick={() => onFilter(key)} className={`rounded-full px-4 py-2 text-sm font-black ${filter === key ? 'bg-[#022444] text-white' : 'border border-slate-300 bg-white text-slate-600'}`}>{label}</button>)}</div>
    <section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">{!overview ? <Loader2 className="mx-auto my-20 h-7 w-7 animate-spin text-[#E12120]" /> : <div className="overflow-x-auto"><ResponsiveTable className="w-full min-w-[760px] text-left"><thead className="bg-slate-50 text-xs font-black uppercase tracking-wider text-slate-400"><tr><th className="p-4">Cliente</th><th className="p-4">Classificação</th><th className="p-4">Ciclo</th><th className="p-4">Cadastro</th><th className="p-4" /></tr></thead><tbody className="divide-y divide-slate-100">{tenants.map(tenant => <tr key={tenant.id} className="hover:bg-slate-50"><td className="p-4"><button onClick={() => onTenant(tenant)} className="text-left"><p className="font-black">{tenant.name}</p><p className="text-sm text-slate-500">{tenant.legal_name || tenant.slug}</p><p className="mt-1 font-mono text-xs text-slate-400">{tenant.tax_id || 'CPF/CNPJ não informado'}</p></button></td><td className="p-4 text-sm font-black">{typeLabel[tenant.customer_type ?? 'TEST']}</td><td className="p-4"><span className={`rounded-full px-2.5 py-1 text-xs font-black ${tenant.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{statusLabel[tenant.status]}</span></td><td className="p-4"><span className={`text-xs font-black ${tenant.profile_complete ? 'text-emerald-700' : 'text-amber-700'}`}>{tenant.profile_complete ? 'COMPLETO' : 'INCOMPLETO'}</span></td><td className="p-4"><button onClick={() => onTenant(tenant)} className="flex items-center gap-2 rounded-xl bg-[#022444] px-3 py-2 text-xs font-black text-white">Abrir <ArrowRight className="h-4 w-4" /></button></td></tr>)}</tbody></ResponsiveTable>{tenants.length === 0 && <p className="py-16 text-center text-sm font-semibold text-slate-500">Nenhum cliente corresponde ao filtro.</p>}</div>}</section>
  </div>
}
...
function SystemHealthView({ health, storageCapacity, onRefresh, onOrganizations }: { health: PlatformSystemHealth | null; storageCapacity: PlatformStorageCapacity | null; onRefresh: () => Promise<void>; onOrganizations: () => void }) {
  ...
  return <div className="mx-auto max-w-[1500px] p-5 sm:p-8"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-[#E12120]">Observabilidade</p><h2 className="mt-2 text-3xl font-black">Saúde da plataforma</h2><p className="mt-2 text-slate-500">Sondagens e inventários com origem e horário informados pelo servidor.</p></div><button onClick={onRefresh} className="flex h-11 shrink-0 items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 text-sm font-black"><RefreshCw className="h-4 w-4" />Atualizar</button></div>{!health ? <Loader2 className="mx-auto my-24 h-8 w-8 animate-spin text-[#E12120]" /> : <><section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{Object.entries(health.totals).map(([key, value]) => <button key={key} onClick={() => openTotal(key)} className="rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#E12120]/40 hover:shadow-md"><p className="text-xs font-black uppercase text-slate-400">{totalLabel(key)}</p><p className="mt-3 text-3xl font-black">{value}</p><p className="mt-4 flex items-center justify-between text-xs font-bold text-slate-500">{key === 'tenants' ? 'Abrir organizações' : 'Investigar fila transacional'}<ArrowRight className="h-4 w-4" /></p></button>)}</section>{storageCapacity && <StorageCapacitySection capacity={storageCapacity} />}<section className="mt-6 grid gap-4 lg:grid-cols-2">{health.components.map(component => <ComponentHealth key={component.key} component={component} />)}</section><p className="mt-5 text-xs font-semibold text-slate-500">Verificado em {formatProductDateTime(health.checked_at)}</p></>}</div>
}
