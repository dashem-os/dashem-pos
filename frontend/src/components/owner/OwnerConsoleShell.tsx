...
export function OwnerConsoleShell({ me }: { me: AuthMe }) {
  ...
  return <div className="min-h-screen bg-[#f4f6f9] text-slate-950 lg:grid lg:grid-cols-[260px_minmax(0,1fr)]">
    <aside className={`${mobileNav ? 'flex' : 'hidden'} fixed inset-y-0 left-0 z-40 w-[280px] flex-col bg-[#081222] p-5 text-white shadow-2xl lg:static lg:flex lg:w-auto lg:shadow-none`}>
      <div className="flex items-center justify-between"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-rose-600 font-black shadow-lg shadow-rose-600/30">D</div><div><p className="font-black">DASHEM</p><p className="text-[10px] font-bold uppercase tracking-[.2em] text-slate-500">Control</p></div></div><button className="rounded-xl border border-white/10 p-2 lg:hidden" onClick={() => setMobileNav(false)}><X className="h-5 w-5" /></button></div>
    ...
    {mobileNav && <button className="fixed inset-0 z-30 bg-slate-950/50 lg:hidden" onClick={() => setMobileNav(false)} />}

    <main className="min-w-0"><header className="flex h-20 items-center justify-between border-b border-slate-200 bg-white px-5 sm:px-8"><div className="flex items-center gap-3"><button className="flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 lg:hidden" onClick={() => setMobileNav(true)}><Menu className="h-5 w-5" /></button><div><p className="text-xs font-bold uppercase tracking-[.16em] text-slate-400">Dashem Control</p><h1 className="text-lg font-black">{title}</h1></div></div><div className="flex gap-2">{view === 'organizations' && <button onClick={() => setCreateOpen(true)} className="flex h-11 items-center gap-2 rounded-xl bg-rose-600 px-4 text-sm font-black text-white shadow-lg shadow-rose-600/20"><Plus className="h-4 w-4" /><span className="hidden sm:inline">Novo cliente</span></button>}<button onClick={signOut} className="flex h-11 items-center gap-2 rounded-xl border border-slate-200 px-3 text-sm font-black text-slate-600"><LogOut className="h-4 w-4" /><span className="hidden xl:inline">Sair</span></button></div></header>
...
function SystemHealthView({ health, onRefresh }: { health: PlatformSystemHealth | null; onRefresh: () => Promise<void> }) {
  return <div className="mx-auto max-w-[1500px] p-5 sm:p-8"><div className="flex items-start justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-rose-600">Observabilidade</p><h2 className="mt-2 text-3xl font-black">Saúde da plataforma</h2><p className="mt-2 text-slate-500">Sondagens executadas agora; nenhum componente é marcado saudável sem evidência.</p></div><button onClick={onRefresh} className="flex h-11 items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 text-sm font-black"><RefreshCw className="h-4 w-4" />Atualizar</button></div>{!health ? <Loader2 className="mx-auto my-24 h-8 w-8 animate-spin text-rose-600" /> : <><section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{Object.entries(health.totals).map(([key, value]) => <article key={key} className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-xs font-black uppercase text-slate-400">{totalLabel(key)}</p><p className="mt-3 text-3xl font-black">{value}</p></article>)}</section><section className="mt-6 grid gap-4 lg:grid-cols-2">{health.components.map(component => <ComponentHealth key={component.key} component={component} />)}</section><p className="mt-5 text-xs font-semibold text-slate-500">Verificado em {new Intl.DateTimeFormat('pt-BR', { dateStyle: 'short', timeStyle: 'medium' }).format(new Date(health.checked_at))}</p></>}</div>
}
