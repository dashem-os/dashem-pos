    ...
    <div className="mt-7">
      ...
      {tab === 'summary' && <div className="grid gap-5 lg:grid-cols-3">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 lg:col-span-3">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h3 className="text-lg font-black">Quotas vigentes e capabilities</h3><p className="mt-1 text-sm text-slate-500">A versão contratual define o que está liberado para este tenant; o plano define apenas os tetos permitidos.</p></div><button type="button" onClick={() => openContract('capabilities')} className="h-10 shrink-0 rounded-xl border border-slate-300 bg-white px-4 text-sm font-black hover:border-[#E12120]">Gerenciar capabilities ({enabledCapabilities})</button></div>
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3"><Limit label="Usuários" value={limits.users ?? detail.subscription?.contracted_user_limit ?? detail.plan?.user_limit} /><Limit label="Dispositivos" value={limits.devices ?? detail.subscription?.contracted_device_limit ?? detail.plan?.terminal_limit} /><Limit label="Unidades" value={limits.units ?? detail.subscription?.contracted_store_limit ?? detail.plan?.store_limit} /></div>
          <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-black uppercase text-amber-800">Storage — referência contratual</p><p className="mt-2 text-2xl font-black">{(limits.storage_mb ?? detail.plan?.storage_limit_mb) ? `${limits.storage_mb ?? detail.plan?.storage_limit_mb} MB` : 'Não informado'}</p></div><span className="w-fit rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-900">NÃO MONITORADO</span></div><p className="mt-2 text-sm font-semibold text-amber-900">O sistema ainda não mede consumo de storage e não bloqueia o tenant por este valor. Este campo é somente metadado contratual.</p>{detail.plan?.storage_limit_mb != null && <p className="mt-1 text-xs text-amber-800">Referência cadastrada no plano: {detail.plan.storage_limit_mb} MB.</p>}</div>
        </section>
      </div>}
...
function ContractEditor({ detail, catalog, niches, plans, initialSection, onManagePlans, onFinance, onSaved }: { detail: PlatformTenantDetail; catalog: CapabilityCatalogItem[]; niches: OwnerNiche[]; plans: ServicePlan[]; initialSection: ContractSection; onManagePlans: () => void; onFinance: () => void; onSaved: () => Promise<void> }) {
  ...
  return <div className="space-y-6">
    ...
    {contractSection === 'capabilities' && <section className="rounded-2xl border border-slate-200 bg-white p-6"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><h4 className="font-black">Capabilities do cliente</h4><p className="mt-1 text-sm text-slate-500">Verde significa liberação efetiva após salvar esta versão; âmbar é apenas sugestão do nicho.</p></div><span className="w-fit rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700">{keys.length} SELECIONADAS</span></div><p className="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm font-semibold text-blue-900">Caminho no Console: Organizações → cliente → Contrato → Capabilities. Clique nos itens e salve uma nova versão auditada.</p><div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{catalog.map(item => <button key={item.key} onClick={() => toggleCapability(item.key)} className={`rounded-xl border-2 p-4 text-left ${keys.includes(item.key) ? 'border-emerald-400 bg-emerald-50' : suggested.has(item.key) ? 'border-[#ffbf00] bg-amber-50' : 'border-slate-200'}`}><div className="flex justify-between gap-3"><span className="font-black">{item.name}</span>{keys.includes(item.key) && <Check className="h-5 w-5 text-emerald-600" />}</div><p className="mt-2 text-sm text-slate-500">{item.description}</p></button>)}</div></section>}
    {contractSection === 'limits' && <section className="rounded-2xl border border-slate-200 bg-white p-6"><h4 className="font-black">Quotas vigentes</h4><p className="mt-1 text-sm text-slate-500">Usuários, dispositivos e unidades são validados pelo backend. Podem ficar abaixo do teto do plano, mas nunca acima.</p><div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3"><NumberInput label="Usuários — aplicado" value={users} max={selectedPlan?.user_limit} onChange={setUsers} /><NumberInput label="Dispositivos — aplicado" value={devices} max={selectedPlan?.terminal_limit} onChange={setDevices} /><NumberInput label="Unidades — aplicado" value={units} max={selectedPlan?.store_limit} onChange={setUnits} /></div><div className="mt-5 rounded-xl border border-amber-300 bg-amber-50 p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><h5 className="font-black">Storage — referência contratual, sem medição</h5><p className="mt-1 text-sm text-amber-900">Este valor ainda não é comparado ao consumo e não produz bloqueio operacional.</p></div><span className="w-fit rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-900">NÃO APLICADO</span></div><div className="mt-3 max-w-sm"><NumberInput label="Referência de storage (MB)" value={storage} min={128} max={selectedPlan?.storage_limit_mb} onChange={setStorage} /></div></div></section>}
...
function Limit({ label, value }: { label: string; value: unknown }) { return <div className="rounded-xl bg-slate-50 p-4"><div className="flex items-center justify-between gap-2"><p className="text-xs font-black uppercase text-slate-400">{label}</p><span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-black text-emerald-700">APLICADO</span></div><p className="mt-2 text-2xl font-black">{value === undefined ? '—' : String(value)}</p></div> }
