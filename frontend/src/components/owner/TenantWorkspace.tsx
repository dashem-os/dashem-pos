...
const storageSize = (value?: number) => value == null ? '—' : value >= 1024 ** 3 ? `${(value / 1024 ** 3).toLocaleString('pt-BR', { maximumFractionDigits: 2 })} GB` : `${(value / 1024 ** 2).toLocaleString('pt-BR', { maximumFractionDigits: 2 })} MB`
const providerStorageSize = (value?: number) => value == null ? '—' : value >= 1_000_000_000 ? `${(value / 1_000_000_000).toLocaleString('pt-BR', { maximumFractionDigits: 2 })} GB` : `${(value / 1_000_000).toLocaleString('pt-BR', { maximumFractionDigits: 2 })} MB`
  ...
  return <div className="mx-auto max-w-[1500px] p-5 text-[#022444] sm:p-8">
    ...
    <div className="mt-7">
      ...
      {tab === 'summary' && <div className="grid gap-5 lg:grid-cols-3">
        <Card icon={Building2} title="Contrato" value={detail.plan?.name || 'Sem plano'} hint={`${detail.niches.length ? detail.niches.map(item => nicheLabel[item]).join(' + ') : 'Nicho não informado'} · versão ${detail.contract?.version ?? '—'}`} onClick={() => openContract('billing')} />
...
function ContractEditor({ detail, catalog, niches, plans, initialSection, onManagePlans, onFinance, onSaved }: { detail: PlatformTenantDetail; catalog: CapabilityCatalogItem[]; niches: OwnerNiche[]; plans: ServicePlan[]; initialSection: ContractSection; onManagePlans: () => void; onFinance: () => void; onSaved: () => Promise<void> }) {
  ...
  const selectPlan = (value: string) => {
    ...
    setStorage(plan.storage_limit_mb == null ? '' : String(plan.storage_limit_mb))
  }
  const save = async () => {
    if (saving) return
  ...
  return <div className="space-y-6">
    ...
    {contractSection === 'billing' && <section className="rounded-2xl border border-slate-200 bg-white p-6">
      ...
      {selectedPlan && <div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900"><p className="font-black">Tetos da oferta {selectedPlan.name}</p><p className="mt-1">{selectedPlan.capability_keys.length} capabilities elegíveis · {selectedPlan.user_limit ?? '∞'} usuários · {selectedPlan.terminal_limit ?? '∞'} dispositivos · {selectedPlan.store_limit ?? '∞'} unidades · {selectedPlan.storage_limit_mb ?? '∞'} MB</p><p className="mt-1 text-xs font-semibold">Selecionar o plano não libera capabilities. A composição com as atividades e a decisão contratual do Owner definem o acesso.</p></div>}
...
function Limit({ label, usage }: { label: string; usage: PlatformTenantDetail['resource_usage'][keyof PlatformTenantDetail['resource_usage']] }) {
  if (!usage) return <div className="rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="text-xs font-black uppercase text-amber-800">{label}</p><p className="mt-2 font-black text-amber-900">Leitura indisponível</p></div>
  const warning = usage.decision === 'WARNING' || usage.decision === 'DENIED' || usage.decision === 'UNKNOWN'
  return <div className={`rounded-xl border p-4 ${warning ? 'border-amber-300 bg-amber-50' : 'border-slate-100 bg-slate-50'}`}>
    <div className="flex items-center justify-between gap-2"><p className={`text-xs font-black uppercase ${warning ? 'text-amber-800' : 'text-slate-400'}`}>{label}</p><span className={`rounded-full px-2 py-1 text-[10px] font-black ${warning ? 'bg-amber-100 text-amber-900' : 'bg-emerald-100 text-emerald-800'}`}>{usage.decision === 'WARNING' ? 'ATENÇÃO' : usage.decision === 'DENIED' ? 'LIMITE EXCEDIDO' : usage.decision === 'UNKNOWN' ? 'SEM TETO' : 'NORMAL'}</span></div>
    <p className="mt-3 text-2xl font-black">{usage.occupied} <span className="text-sm text-slate-400">/ {usage.contracted ?? 'sem teto'}</span></p>
    <dl className="mt-3 grid grid-cols-3 gap-2 text-xs"><div><dt className="font-bold text-slate-400">Configurados</dt><dd className="mt-1 font-black">{usage.configured}</dd></div><div><dt className="font-bold text-slate-400">Reservados</dt><dd className="mt-1 font-black">{usage.reserved}</dd></div><div><dt className="font-bold text-slate-400">Disponíveis</dt><dd className="mt-1 font-black">{usage.available ?? '—'}</dd></div></dl>
    {warning && <p className="mt-3 text-xs font-semibold text-amber-900">{usage.reason}</p>}
  </div>
}
...
function StorageQuotaPanel({ usage, contractLimitMb, planLimitMb, working, onBootstrap, onReconcile }: { usage: PlatformTenantDetail['storage_usage']; contractLimitMb?: number; planLimitMb?: number; working: boolean; onBootstrap: () => void; onReconcile: () => void }) {
  return <div className={`mt-4 rounded-xl border p-4 ${usage.enforcement_active ? 'border-emerald-300 bg-emerald-50' : 'border-amber-300 bg-amber-50'}`}>
    ...
    <div className="mt-4 rounded-lg border border-slate-200 bg-white/70 p-3"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black uppercase">Capacidade física compartilhada · Supabase</p><p className="mt-1 text-xs font-semibold">{usage.provider_capacity.reason}</p></div><button disabled={working} onClick={usage.provider_capacity.configured ? onReconcile : onBootstrap} className="h-9 rounded-lg bg-[#022444] px-3 text-xs font-black text-white disabled:opacity-40">{working ? 'Processando…' : usage.provider_capacity.configured ? 'Reconciliar agora' : 'Conectar Supabase Storage'}</button></div>{usage.provider_capacity.configured && <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-4"><div><dt className="text-slate-500">Capacidade declarada</dt><dd className="font-black">{providerStorageSize(usage.provider_capacity.capacity_bytes)}</dd></div><div><dt className="text-slate-500">Uso físico</dt><dd className="font-black">{providerStorageSize(usage.provider_capacity.used_bytes)}</dd></div><div><dt className="text-slate-500">Margem reservada</dt><dd className="font-black">{providerStorageSize(usage.provider_capacity.reserved_margin_bytes)}</dd></div><div><dt className="text-slate-500">Disponível global</dt><dd className="font-black">{providerStorageSize(usage.provider_capacity.available_bytes)}</dd></div></dl>}<p className="mt-2 text-[11px] font-semibold text-slate-500">Capacidade física exibida em unidades decimais, conforme a franquia do provedor.</p><p className="mt-3 border-t border-slate-200 pt-2 text-xs font-semibold text-amber-800">Egress: não instrumentado. {usage.provider_capacity.egress_reason}</p></div>
  </div>
}
