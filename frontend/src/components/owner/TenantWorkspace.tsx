...
export function TenantWorkspace({ tenant, onBack, onManagePlans, onChanged }: { tenant: PlatformTenantSummary; onBack: () => void; onManagePlans: () => void; onChanged: () => void }) {
...
function ContractEditor({ detail, catalog, niches, plans, onManagePlans, onSaved }: { detail: PlatformTenantDetail; catalog: CapabilityCatalogItem[]; niches: OwnerNiche[]; plans: ServicePlan[]; onManagePlans: () => void; onSaved: () => Promise<void> }) {
  const limits = detail.contract?.limits || {}
  ...
  const billing = {
    contact_name: savedBilling.contact_name || primaryContact?.full_name || contractualAdmin?.full_name || '',
    email: savedBilling.email || primaryContact?.email || detail.profile?.company_email || contractualAdmin?.email || '',
    phone: savedBilling.phone || primaryContact?.phone || detail.profile?.company_phone || '',
  ...
  if (editing && selectablePlans.length === 0) return <section className="rounded-2xl border border-[#ffbf00] bg-amber-50 p-8 text-center"><WalletCards className="mx-auto h-9 w-9 text-[#8a6100]" /><h3 className="mt-4 text-xl font-black">O catálogo ainda não possui planos comerciais</h3><p className="mx-auto mt-2 max-w-2xl text-sm text-slate-600">Cadastre ao menos um plano para definir mensalidade, limites e contrato deste cliente. Os dados do tenant permanecem preservados.</p><button type="button" onClick={onManagePlans} className="mt-5 inline-flex h-11 items-center gap-2 rounded-xl bg-[#E12120] px-5 font-black text-white"><Plus className="h-4 w-4" />Cadastrar plano comercial</button></section>
  const save = async () => {
    if (saving) return
    ...
    try {
      await updateOwnerTenantContract(detail.tenant.id, {
        ...
        subscription_status: subscriptionStatus, billing_status: billingStatus,
        next_due_date: nextDueDate || undefined, reason,
      })
      await onSaved()
      setEditing(false)
    } catch (cause) {
  ...
  return <div className="space-y-6"><div className="flex items-center justify-between"><div><h3 className="text-xl font-black">Editar contrato e mensalidade</h3><p className="mt-1 text-sm text-slate-500">Cada salvamento cria uma nova versão auditada.</p></div>{detail.contract && <button onClick={() => setEditing(false)} className="rounded-xl border border-slate-200 p-2"><X className="h-5 w-5" /></button>}</div><nav className="flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white p-2">{([['billing', 'Plano e cobrança'], ['models', 'Modelos de negócio'], ['capabilities', 'Capabilities'], ['limits', 'Limites']] as Array<[typeof contractSection, string]>).map(([key, label]) => <button type="button" key={key} onClick={() => setContractSection(key)} className={`rounded-xl px-4 py-3 text-sm font-black ${contractSection === key ? 'bg-[#022444] text-white' : 'text-slate-500 hover:bg-slate-100'}`}>{label}</button>)}</nav>{error && <p className="rounded-xl border border-[#ffbf00] bg-amber-50 p-4 text-sm font-bold">{error}</p>}{contractSection === 'billing' && <section className="rounded-2xl border border-slate-200 bg-white p-6"><h4 className="font-black">Plano e cobrança SaaS</h4><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Select label="Plano" value={planId} onChange={setPlanId} options={[["", "Selecione"], ...plans.map(plan => [plan.id, `${plan.name} · ${money(plan.monthly_price)}`] as [string, string])]} /><TextField label="Mensalidade negociada (R$)" value={amount} onChange={setAmount} /><NumberInput label="Dia de vencimento" value={billingDay} min={1} max={28} onChange={setBillingDay} /><TextField label="Próximo vencimento" type="date" value={nextDueDate} onChange={setNextDueDate} /><Select label="Assinatura" value={subscriptionStatus} onChange={value => setSubscriptionStatus(value as SubscriptionStatus)} options={[["PENDING", "Pendente"], ["TRIAL", "Avaliação"], ["ACTIVE", "Ativa"], ["PAUSED", "Pausada"], ["CANCELED", "Cancelada"]]} /><Select label="Situação financeira" value={billingStatus} onChange={setBillingStatus} options={[["PENDING", "Pendente"], ["CURRENT", "Em dia"], ["OVERDUE", "Em atraso"], ["WAIVED", "Isenta"]]} /><TextField label="Contato de cobrança" value={contactName} onChange={setContactName} /><TextField label="E-mail de cobrança" value={email} onChange={setEmail} /><TextField label="Telefone de cobrança" value={formatBrazilianPhone(phone)} onChange={value => setPhone(onlyDigits(value, 11))} /></div></section>}{contractSection === 'models' && <section className="rounded-2xl border border-slate-200 bg-white p-6"><h4 className="font-black">Definir segmentos do negócio</h4><p className="mt-1 text-sm text-slate-500">Defina um ou mais segmentos contratados. Combinações híbridas são permitidas e podem ser alteradas.</p><div className="mt-5 grid gap-3 md:grid-cols-3">{niches.map(niche => <button key={niche.key} onClick={() => toggleNiche(niche.key)} className={`rounded-xl border-2 p-4 text-left ${selectedNiches.includes(niche.key) ? 'border-[#E12120] bg-red-50' : 'border-slate-200'}`}><div className="flex justify-between gap-3"><span className="font-black">{niche.name}</span>{selectedNiches.includes(niche.key) && <Check className="h-5 w-5 text-[#E12120]" />}</div><p className="mt-2 text-sm text-slate-500">{niche.description}</p></button>)}</div></section>}{contractSection === 'capabilities' && <section className="rounded-2xl border border-slate-200 bg-white p-6"><h4 className="font-black">Capabilities contratadas</h4><p className="mt-1 text-sm text-slate-500">Âmbar indica sugestão; verde indica contratação efetiva.</p><div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{catalog.map(item => <button key={item.key} onClick={() => toggleCapability(item.key)} className={`rounded-xl border-2 p-4 text-left ${keys.includes(item.key) ? 'border-emerald-400 bg-emerald-50' : suggested.has(item.key) ? 'border-[#ffbf00] bg-amber-50' : 'border-slate-200'}`}><div className="flex justify-between gap-3"><span className="font-black">{item.name}</span>{keys.includes(item.key) && <Check className="h-5 w-5 text-emerald-600" />}</div><p className="mt-2 text-sm text-slate-500">{item.description}</p></button>)}</div></section>}{contractSection === 'limits' && <section className="rounded-2xl border border-slate-200 bg-white p-6"><h4 className="font-black">Limites contratados</h4><div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><NumberInput label="Usuários" value={users} max={selectedPlan?.user_limit} onChange={setUsers} /><NumberInput label="Dispositivos" value={devices} max={selectedPlan?.terminal_limit} onChange={setDevices} /><NumberInput label="Unidades" value={units} max={selectedPlan?.store_limit} onChange={setUnits} /><NumberInput label="Storage (MB)" value={storage} min={128} max={selectedPlan?.storage_limit_mb} onChange={setStorage} /></div></section>}<TextField label="Motivo da alteração" value={reason} onChange={setReason} /><button onClick={save} disabled={saving} className="flex h-11 items-center gap-2 rounded-xl bg-[#E12120] px-5 font-black text-white disabled:opacity-40">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{saving ? 'Salvando…' : 'Salvar nova versão'}</button>{error && <p className="text-sm font-bold text-[#8a6100]">A alteração não foi salva. {error}</p>}</div>
}
