...
function AccessTab({ detail, onSaved }: { detail: PlatformTenantDetail; onSaved: (message: string) => Promise<void> }) {
  ...
  return <div className="space-y-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-xl font-black">Acessos do tenant</h2><p className="text-sm text-slate-500">Conceda, suspenda, reative ou revogue com auditoria.</p></div><button onClick={() => setOpen(value => !value)} className="flex h-11 items-center gap-2 rounded-xl bg-rose-600 px-4 text-sm font-black text-white"><Plus className="h-4 w-4" />Conceder acesso</button></div>{error && <p className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-bold text-red-700">{error}</p>}{open && <form onSubmit={invite} className="rounded-2xl border border-slate-200 bg-white p-5"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Field label="Nome" value={form.full_name} onChange={value => setForm(current => ({ ...current, full_name: value }))} /><Field label="E-mail" value={form.email} onChange={value => setForm(current => ({ ...current, email: value }))} type="email" /><label className="text-sm font-black">Papel<select className={`mt-2 ${inputClass}`} value={form.role} onChange={e => setForm(current => ({ ...current, role: e.target.value }))}><option value="TENANT_OWNER">Owner do cliente</option><option value="ADMIN">Administrador</option><option value="MANAGER">Gerente</option><option value="AUDITOR">Auditor</option><option value="CASHIER">Caixa</option><option value="OPERATOR">Operador</option></select></label><label className="text-sm font-black">Unidade<select className={`mt-2 ${inputClass}`} value={form.store_id} onChange={e => setForm(current => ({ ...current, store_id: e.target.value }))}><option value="">Todas as unidades</option>{detail.stores.map(store => <option key={store.id} value={store.id}>{store.name}</option>)}</select></label></div><button disabled={saving || !form.full_name || !form.email.includes('@')} className="mt-5 h-11 rounded-xl bg-slate-950 px-5 text-sm font-black text-white">{saving ? 'Enviando…' : 'Enviar convite'}</button></form>}<label className="block max-w-2xl text-sm font-black">Motivo usado nas ações de acesso<input value={reason} onChange={e => setReason(e.target.value)} className={`mt-2 ${inputClass}`} /></label><div className="overflow-hidden rounded-2xl border border-slate-200 bg-white"><table className="w-full min-w-[760px] text-left"><thead className="bg-slate-50 text-xs font-black uppercase text-slate-400"><tr><th className="p-4">Usuário</th><th className="p-4">Papel</th><th className="p-4">Escopo</th><th className="p-4">Estado</th><th className="p-4">Ações</th></tr></thead><tbody className="divide-y divide-slate-100">{detail.accesses.map(access => <tr key={access.membership_id}><td className="p-4"><p className="font-black">{access.full_name}</p><p className="text-sm text-slate-500">{access.email}</p></td><td className="p-4 text-sm font-bold">{access.role}</td><td className="p-4 text-sm">{access.store_name || 'Tenant inteiro'}</td><td className="p-4"><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-black">{access.status}</span></td><td className="p-4"><div className="flex gap-2">{access.status === 'ACTIVE' ? <><button onClick={() => change(access, 'SUSPENDED')} title="Suspender" className="rounded-lg border border-amber-200 p-2 text-amber-700"><Ban className="h-4 w-4" /></button><button onClick={() => change(access, 'REVOKED')} title="Revogar" className="rounded-lg border border-red-200 p-2 text-red-700"><ShieldOff className="h-4 w-4" /></button></> : <button onClick={() => change(access, 'ACTIVE')} className="rounded-lg border border-emerald-200 px-3 py-2 text-xs font-black text-emerald-700">Reativar</button>}</div></td></tr>)}</tbody></table>{detail.accesses.length === 0 && <p className="p-8 text-center text-sm font-semibold text-slate-500">Nenhum acesso concedido.</p>}</div></div>
}
