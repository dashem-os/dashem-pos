...
function StorageQuotaPanel({ usage, contractLimitMb, working, onBootstrap, onReconcile, onCapacity }: { usage: PlatformTenantDetail['storage_usage']; contractLimitMb?: number; working: boolean; onBootstrap: () => void; onReconcile: () => void; onCapacity: () => void }) {
  ...
  return <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
    ...
    <dl className="mt-4 grid gap-2 border-t border-slate-100 pt-3 text-xs sm:grid-cols-3"><div><dt className="text-slate-500">Código do estado</dt><dd className="font-mono font-black">{usage.status_code}</dd></div><div><dt className="text-slate-500">Horário observado</dt><dd className="font-black">{usage.measured_at ? new Date(usage.measured_at).toLocaleString('pt-BR') : '—'}</dd></div><div><dt className="text-slate-500">Id da medição</dt><dd className="break-all font-mono font-black">{usage.measurement_id ?? '—'}</dd></div></dl>
