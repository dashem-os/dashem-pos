    ...
    <div className="mt-7">
      ...
      {tab === 'summary' && <div className="grid gap-5 lg:grid-cols-3">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 lg:col-span-3">
          ...
          <StorageQuotaPanel usage={detail.storage_usage} contractLimitMb={limits.storage_mb} working={storageWorking} onBootstrap={() => void runStorage(true)} onReconcile={() => void runStorage(false)} onCapacity={onCapacity} />
        </section>
      </div>}
...
function QuotaFactsTable({ usage }: { usage: PlatformTenantDetail['resource_usage'] }) {
  ...
  return <div className="mt-5 overflow-x-auto rounded-xl border border-slate-200"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-slate-50 text-xs font-black uppercase text-slate-400"><tr><th className="p-4">Recurso</th><th className="p-4">Contratado</th><th className="p-4">Configurado</th><th className="p-4">Reservado</th><th className="p-4">Disponível</th><th className="p-4">Excedente</th><th className="p-4">Estado</th></tr></thead><tbody className="divide-y divide-slate-100">{rows.map(row => <tr key={row.key}><td className="p-4 font-black">{row.label}</td><td className="p-4">{row.value?.contracted ?? '—'}</td><td className="p-4">{row.value?.configured ?? '—'}</td><td className="p-4">{row.value ? row.value.reservation_supported ? row.value.reserved : 'Não aplicável' : '—'}</td><td className="p-4">{row.value?.available ?? '—'}</td><td className="p-4 font-black">{row.value?.overage ?? '—'}</td><td className="p-4 font-mono text-xs">{row.value?.compliance_status ?? 'UNAVAILABLE'}</td></tr>)}</tbody></table></div>
}
function StorageStatus({ usage }: { usage: PlatformTenantDetail['storage_usage'] }) {
  return <span className="w-fit rounded-full bg-slate-100 px-3 py-1 font-mono text-xs font-black text-slate-700">{usage.measurement_status}</span>
}
function StorageQuotaPanel({ usage, contractLimitMb, working, onBootstrap, onReconcile, onCapacity }: { usage: PlatformTenantDetail['storage_usage']; contractLimitMb?: number; working: boolean; onBootstrap: () => void; onReconcile: () => void; onCapacity: () => void }) {
  ...
  return <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-black uppercase">Storage do tenant</p><p className="mt-2 text-2xl font-black">{contractLimitMb ? `${contractLimitMb} MiB` : '—'}</p></div><div className="flex flex-wrap gap-2"><StorageStatus usage={usage} /><span className="rounded-full bg-slate-100 px-3 py-1 font-mono text-xs font-black text-slate-700">{usage.quota_status}</span></div></div>
    ...
    <dl className="mt-4 grid gap-2 border-t border-slate-100 pt-3 text-xs sm:grid-cols-3"><div><dt className="text-slate-500">Código do estado</dt><dd className="font-mono font-black">{usage.status_code}</dd></div><div><dt className="text-slate-500">Horário observado</dt><dd className="font-black">{formatProductDateTime(usage.measured_at)}</dd></div><div><dt className="text-slate-500">Id da medição</dt><dd className="break-all font-mono font-black">{usage.measurement_id ?? '—'}</dd></div></dl>
    {usage.watermark && <p className="mt-3 break-all font-mono text-[11px] text-slate-500">{usage.watermark}</p>}
