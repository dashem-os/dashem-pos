    ...
    <div className="mt-7">
      ...
      {tab === 'summary' && <div className="grid gap-5 lg:grid-cols-3">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 lg:col-span-3">
          ...
          <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-black uppercase text-amber-800">Storage — referência contratual</p><p className="mt-2 text-2xl font-black">{limits.storage_mb ? `${limits.storage_mb} MB` : 'Não informado no contrato'}</p></div><span className="w-fit rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-900">NÃO MONITORADO</span></div><p className="mt-2 text-sm font-semibold text-amber-900">O sistema ainda não mede consumo de storage e não bloqueia o tenant por este valor. Este campo é somente metadado contratual.</p>{detail.plan?.storage_limit_mb != null && <p className="mt-1 text-xs text-amber-800">Teto da oferta no plano: {detail.plan.storage_limit_mb} MB.</p>}</div>
