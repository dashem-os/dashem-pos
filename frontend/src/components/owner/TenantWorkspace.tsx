    ...
    <div className="mt-7">
      ...
      {tab === 'summary' && <div className="grid gap-5 lg:grid-cols-3">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 lg:col-span-3">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3"><Limit label="Quota contratual de usuários" value={limits.users} /><Limit label="Quota contratual de dispositivos" value={limits.devices} /><Limit label="Quota contratual de unidades" value={limits.units} /></div>
...
function Limit({ label, value }: { label: string; value: unknown }) { return <div className="rounded-xl bg-slate-50 p-4"><p className="text-xs font-black uppercase text-slate-400">{label}</p><p className="mt-2 text-2xl font-black">{value === undefined ? 'Não informada' : String(value)}</p><p className="mt-1 text-xs font-semibold text-slate-400">Uso real ainda não apresentado neste painel.</p></div> }
