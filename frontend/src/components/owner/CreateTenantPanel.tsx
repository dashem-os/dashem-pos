...
type FormState = {
  ...
  users: string; devices: string; units: string; storageMib: string
...
const initialForm: FormState = {
  ...
  users: '', devices: '', units: '', storageMib: '', adminName: '', adminEmail: '',
}
...
export function CreateTenantPanel({ onClose, onCreated, onManagePlans }: { onClose: () => void; onCreated: () => void; onManagePlans: () => void }) {
  ...
  const errorsForStep = () => {
    ...
    if (step === 0) {
      ...
      if (!Number.isInteger(Number(form.billingDay)) || Number(form.billingDay) < 1 || Number(form.billingDay) > 28) required.billingDay = 'Informe um dia entre 1 e 28.'
    ...
    if (step === 4) {
      ...
      if (!form.storageMib || Number(form.storageMib) < 128 || (selectedPlan?.storage_limit_mib && Number(form.storageMib) > selectedPlan.storage_limit_mib)) required.storageMib = 'Revise a quota de armazenamento.'
    }
  ...
  const selectPlan = (plan: ServicePlan) => setForm(current => ({ ...current, planId: plan.id, capabilityKeys: [], monthlyAmount: Number(plan.monthly_price || 0).toFixed(2).replace('.', ','), users: plan.user_limit == null ? current.users : String(plan.user_limit), devices: plan.terminal_limit == null ? current.devices : String(plan.terminal_limit), units: plan.store_limit == null ? current.units : String(plan.store_limit), storageMib: plan.storage_limit_mib == null ? current.storageMib : String(plan.storage_limit_mib) }))
  const submit = async () => {
    ...
    try {
      await provisionPlatformTenant({
        ...
        quotas: { users: Number(form.users), devices: Number(form.devices), units: Number(form.units), storage_mib: Number(form.storageMib) }, capability_keys: form.capabilityKeys,
  ...
  return <div className="fixed inset-0 z-50 flex items-stretch justify-center bg-[#022444]/70 p-0 backdrop-blur-sm sm:p-5">
    ...
    <section className="relative flex h-full w-full max-w-[1450px] flex-col overflow-hidden bg-white text-[#022444] shadow-2xl sm:h-[calc(100dvh-2.5rem)] sm:rounded-3xl">
      <header className="flex shrink-0 items-start justify-between gap-3 border-b border-slate-200 px-5 py-5 sm:px-8"><div><p className="text-xs font-black uppercase tracking-[.16em] text-[#E12120]">OWNER · PROVISIONAMENTO</p><h2 className="mt-1 text-2xl font-black">Novo cliente SaaS</h2><p className="mt-1 text-sm text-slate-500">Cadastro, contrato, sugestões e administrador em um fluxo revisável.</p></div><button onClick={onClose} className="rounded-xl border border-slate-200 p-2 text-slate-500"><X /></button></header>
      <ol className="flex shrink-0 gap-2 overflow-x-auto border-b border-slate-200 px-5 py-3 sm:px-8">{steps.map((label, index) => <li key={label} className={`flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-xs font-black ${index === step ? 'bg-[#022444] text-white' : index < step ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>{index < step && <Check className="h-3.5 w-3.5" />}{label}</li>)}</ol>
      <div className="min-h-0 flex-1 overflow-y-auto p-5 sm:p-8">{loadingCatalog ? <Loader2 className="mx-auto mt-24 h-8 w-8 animate-spin text-[#E12120]" /> : <>
        ...
        {step === 0 && <div className="space-y-7"><Section title="Empresa e contrato"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Field id="monthlyAmount" label="Mensalidade negociada (R$)" value={form.monthlyAmount} inputMode="decimal" onChange={value => set('monthlyAmount', value)} /><NumberField id="billingDay" label="Dia de vencimento" value={form.billingDay} min={1} max={28} maxHint="Máximo contratual: 28" error={fieldErrors.billingDay} onChange={value => set('billingDay', value)} />
...
function NumberField({ id, label, value, onChange, min = 1, max, maxHint, error }: { id: string; label: string; value: string; onChange: (value: string) => void; min?: number; max?: number; maxHint?: string; error?: string }) { return <label className="text-sm font-black">{label === 'Armazenamento (MiB)' ? 'Armazenamento (MiB) — quota contratual' : label}<input id={`owner-${id}`} type="text" inputMode="numeric" value={value} onChange={event => onChange(digits(event.target.value).replace(/^0+(?=\d)/, ''))} className={`${inputClass} ${error ? 'border-[#ffbf00] focus:border-[#ffbf00] focus:ring-amber-100' : 'border-slate-300 focus:border-[#E12120] focus:ring-red-100'}`} />{max && <span className="mt-1 block text-xs text-slate-400">{maxHint || `Teto do plano: ${max}`}</span>}{min > 1 && <span className="mt-1 block text-xs text-slate-400">Mínimo: {min}</span>}{error && <span className="mt-1 block text-xs font-bold text-[#8a6100]">{error}</span>}</label> }
