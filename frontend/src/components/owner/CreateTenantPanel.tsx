...
type FormState = {
  ...
  billingName: string; billingEmail: string; billingPhone: string; monthlyAmount: string
...
const initialForm: FormState = {
  ...
  billingName: '', billingEmail: '', billingPhone: '', monthlyAmount: '0,00',
  ...
  const submit = async () => {
    ...
    try {
      await provisionPlatformTenant({
        ...
        billing: { contact_name: form.billingName.trim(), email: form.billingEmail.trim(), phone: form.billingPhone.trim() || undefined, monthly_amount: moneyNumber(form.monthlyAmount), billing_day: 1 },
    ...
    <section className="relative flex h-full w-full max-w-[1450px] flex-col overflow-hidden bg-white text-[#022444] shadow-2xl sm:h-[calc(100vh-2.5rem)] sm:rounded-3xl">
      ...
      <div className="flex-1 overflow-y-auto p-5 sm:p-8">{loadingCatalog ? <Loader2 className="mx-auto mt-24 h-8 w-8 animate-spin text-[#E12120]" /> : <>
        ...
        {step === 0 && <div className="space-y-7"><Section title="Empresa e contrato"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Field id="monthlyAmount" label="Mensalidade negociada (R$)" value={form.monthlyAmount} inputMode="decimal" onChange={value => set('monthlyAmount', value)} /><div><p className="text-sm font-black">Vencimento da assinatura</p><div className="mt-2 flex h-11 items-center rounded-xl border border-slate-200 bg-slate-50 px-3 font-black">Todo dia 1</div></div>
