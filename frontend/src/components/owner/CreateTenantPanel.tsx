...
type FormState = {
  ...
  billingName: string; billingEmail: string; billingPhone: string; monthlyAmount: string; billingDay: number
...
const steps = ['Cadastro', 'Nichos', 'Plano', 'Capabilities', 'Limites', 'Administrador']
...
export function CreateTenantPanel({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const errorsForStep = () => {
    ...
    if (step === 4) {
      if (form.users < 1 || (selectedPlan?.user_limit && form.users > selectedPlan.user_limit)) required.users = 'Revise o limite de usuários.'
      if (form.devices < 1 || (selectedPlan?.terminal_limit && form.devices > selectedPlan.terminal_limit)) required.devices = 'Revise o limite de dispositivos.'
      if (form.units < 1 || (selectedPlan?.store_limit && form.units > selectedPlan.store_limit)) required.units = 'Revise o limite de unidades.'
      if (form.storageMb < 128 || (selectedPlan?.storage_limit_mb && form.storageMb > selectedPlan.storage_limit_mb)) required.storageMb = 'Revise o limite de storage.'
    }
  ...
  const selectPlan = (plan: ServicePlan) => setForm(current => ({ ...current, planId: plan.id, monthlyAmount: Number(plan.monthly_price || 0).toFixed(2).replace('.', ','), users: Math.min(current.users, plan.user_limit ?? current.users), devices: Math.min(current.devices, plan.terminal_limit ?? current.devices), units: Math.min(current.units, plan.store_limit ?? current.units), storageMb: Math.min(current.storageMb, plan.storage_limit_mb ?? current.storageMb) }))
  const submit = async () => {
    ...
    try {
      await provisionPlatformTenant({
        ...
        quotas: { users: form.users, devices: form.devices, units: form.units, storage_mb: form.storageMb }, capability_keys: form.capabilityKeys,
        billing: { contact_name: form.billingName.trim(), email: form.billingEmail.trim(), phone: form.billingPhone.trim() || undefined, monthly_amount: moneyNumber(form.monthlyAmount), billing_day: form.billingDay },
    ...
    <section className="relative flex h-full w-full max-w-[1450px] flex-col overflow-hidden bg-white text-[#022444] shadow-2xl sm:h-[calc(100vh-2.5rem)] sm:rounded-3xl">
      ...
      <div className="flex-1 overflow-y-auto p-5 sm:p-8">{loadingCatalog ? <Loader2 className="mx-auto mt-24 h-8 w-8 animate-spin text-[#E12120]" /> : <>
        ...
        {step === 0 && <div className="space-y-7"><Section title="Empresa e contrato"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Field id="companyPhone" label="Telefone da empresa" value={form.companyPhone} error={fieldErrors.companyPhone} onChange={value => set('companyPhone', value)} />
          ...
          <Field id="storeName" label="Nome da unidade" value={form.storeName} error={fieldErrors.storeName} onChange={value => set('storeName', value)} /><Field id="storeCode" label="Código" value={form.storeCode} onChange={value => set('storeCode', value.toUpperCase().replace(/[^A-Z0-9_-]/g, ''))} /><Field id="postalCode" label="CEP" value={form.postalCode} error={fieldErrors.postalCode} inputMode="numeric" onChange={value => set('postalCode', digits(value, 8))} /><Field id="street" label="Logradouro" value={form.street} error={fieldErrors.street} onChange={value => set('street', value)} />
        ...
        {step === 1 && <div><h3 className="text-xl font-black">Atividades do cliente</h3><p className="mt-2 max-w-3xl text-sm text-slate-500">Selecione nenhuma, uma ou várias. Os nichos apenas sugerem uma configuração inicial; não bloqueiam capabilities nem modelos híbridos.</p><ChoiceGrid>{niches.map(niche => <ChoiceCard key={niche.key} selected={form.niches.includes(niche.key)} title={niche.name} description={niche.description} onClick={() => toggleNiche(niche.key)} footer={`${niche.required_capabilities.length} recursos de partida`} />)}</ChoiceGrid></div>}
        ...
        {step === 3 && <div><h3 className="text-xl font-black">Prévia de capabilities</h3><p className="mt-2 text-sm text-slate-500">As sugestões aparecem destacadas, mas todo o catálogo continua disponível para contratação.</p><div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{capabilities.map(capability => <button key={capability.key} type="button" onClick={() => toggleCapability(capability.key)} className={`rounded-xl border-2 p-4 text-left transition ${form.capabilityKeys.includes(capability.key) ? 'border-emerald-400 bg-emerald-50' : suggestedKeys.has(capability.key) ? 'border-[#ffbf00] bg-amber-50' : 'border-slate-200 bg-white'}`}><div className="flex items-start justify-between gap-3"><p className="font-black">{capability.name}</p>{form.capabilityKeys.includes(capability.key) && <span className="rounded-full bg-emerald-600 p-1 text-white"><Check className="h-3.5 w-3.5" /></span>}</div><p className="mt-2 text-sm text-slate-600">{capability.description}</p><p className="mt-3 text-xs font-black uppercase text-slate-400">{suggestedKeys.has(capability.key) ? 'Sugerida pelos nichos' : 'Catálogo geral'}</p></button>)}</div></div>}
...
function NumberField({ id, label, value, onChange, min = 1, max, error }: { id: string; label: string; value: number; onChange: (value: number) => void; min?: number; max?: number; error?: string }) { return <label className="text-sm font-black">{label}<input id={`owner-${id}`} type="number" min={min} max={max} value={value} onChange={event => onChange(Number(event.target.value))} className={`${inputClass} ${error ? 'border-[#ffbf00] focus:border-[#ffbf00] focus:ring-amber-100' : 'border-slate-300 focus:border-[#E12120] focus:ring-red-100'}`} />{max && <span className="mt-1 block text-xs text-slate-400">Teto do plano: {max}</span>}{error && <span className="mt-1 block text-xs font-bold text-[#8a6100]">{error}</span>}</label> }
