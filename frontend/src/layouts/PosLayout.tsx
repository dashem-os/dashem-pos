  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 flex flex-col font-sans selection:bg-brand selection:text-brand-contrast pb-[calc(9rem+env(safe-area-inset-bottom))] sm:pb-[calc(6rem+env(safe-area-inset-bottom))] lg:pb-0">
      ...
      <header className="min-h-14 flex-wrap gap-2 py-2 px-3 sm:px-6 bg-white border-b border-slate-200 flex items-center justify-between shrink-0 z-30 select-none shadow-xs">
        ...
        <div className="flex min-w-0 items-center space-x-3">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {/*
                O topo da operação responde duas coisas: qual caixa e quem está
                nele. "Frente de Caixa" repetia o que a marca já diz, e cada
                selo a mais é uma decisão a mais antes de vender (ADR-034).
              */}
              <span className="rounded-full border border-slate-200 bg-slate-100 px-2 py-0.5 text-xs font-bold text-slate-600">
                {register?.name || 'Terminal'} · {isCashOpen ? 'Aberto' : canReadCash ? 'Caixa fechado' : 'Caixa indisponível'}
              </span>
            </div>
            {/*
        ...
        {/*
          Vender não precisa de estado de rede, saldo de abertura, fechamento de
          caixa, mesas, gestão e saída competindo no topo. Rede só aparece quando
          está ruim — dizer "API acessível" é dizer que nada aconteceu. As ações
          administrativas ficam num menu, subordinadas à venda (ADR-034).

          Mesas continua fora do menu quando é operação de salão: ali ela é a
          ação principal do turno, não administração.
        */}
        <div className="flex max-w-full flex-wrap items-center gap-2">
          {connectionState !== 'ONLINE' && (
            <div className="flex items-center gap-1.5 rounded-xl border border-amber-200 bg-amber-50 px-2.5 py-1 text-xs font-bold text-amber-800">
              <WifiOff className="h-3.5 w-3.5" />
              <span>{connectionState === 'OFFLINE' ? 'Sem rede' : 'Rede instável'}</span>
            </div>
          )}

          {activities.includes('FOOD_SERVICE') && permissions.includes('table.read') && 'table_service' in capabilities && !managementValidation && <button
            onClick={() => navigateTo('/tables')}
            className="flex h-9 items-center space-x-1.5 rounded-xl border border-orange-200 bg-orange-50 px-3.5 text-xs font-bold text-orange-800 transition-colors hover:bg-orange-100 active:scale-95"
          >
            <UtensilsCrossed className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">Mesas</span>
          </button>}

          <RowActions label="Mais ações do terminal">
            {isCashOpen && canCloseCash && (
              <RowAction icon={Lock} onClick={() => { setClosingBalanceInput(''); setIsCloseCashOpen(true) }}>
                Fechar caixa
              </RowAction>
            )}
            {activities.includes('FOOD_SERVICE') && permissions.includes('table.read') && 'table_service' in capabilities && managementValidation && (
              <RowAction icon={UtensilsCrossed} onClick={() => navigateTo('/manage?module=tables')}>
                Configurar mesas
              </RowAction>
            )}
            {managementAvailable && !managementValidation && (
              <RowAction icon={LayoutDashboard} onClick={() => navigateTo('/manage')}>
      ...
      {/*
        O aviso continua: quem valida o PDV com identidade administrativa precisa
        saber que a venda é real e auditada. O que mudou foi o peso — uma linha,
        não uma faixa com botão disputando a atenção de quem vai vender.
      */}
      {managementValidation && (
        <div role="status" className="border-b border-sky-200 bg-sky-50 px-4 py-1.5 text-[11px] text-sky-900 sm:px-6">
          <strong>Acesso gerencial.</strong> Você está validando o PDV com sua identidade administrativa; ações executadas são reais e auditadas.
          {' '}
          <button onClick={() => navigateTo('/manage')} className="font-black text-sky-800 underline underline-offset-2">Voltar à Gestão</button>
        </div>
      )}
        ...
        <main className="mx-auto flex w-full max-w-[1920px] flex-1 flex-col gap-4 overflow-hidden p-3 sm:gap-5 sm:p-5 lg:flex-row">
          ...
          <div className="flex-1 flex flex-col space-y-3 overflow-y-auto min-w-0 pr-0.5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {/*
                O caixa não reafirma a cada venda em qual indústria trabalha. A
                atividade é contratação do negócio, resolvida por precedência, e
                trocá-la é decisão de gestão — não uma escolha entre Coca-Cola e
                Coca-Cola (ADR-034).
              */}
            </div>
