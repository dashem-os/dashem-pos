...
/** Label of the action that moves the ticket to its next status. */
...
/** What the kitchen reads on the badge, in place of the raw status enum. */
const STATUS_LABELS: Record<api.ProductionTicketProjection['ticket']['status'], string> = { NEW: 'Novo', ACCEPTED: 'Aceito', PREPARING: 'Em preparo', READY: 'Pronto', DELIVERED: 'Entregue', CANCELED: 'Cancelado' }

const STATUS_TONES: Record<api.ProductionTicketProjection['ticket']['status'], string> = {
...
function SelectedKdsShell({ selection }: { selection: OperationalSelection }) {
  ...
  return (
    <main className="min-h-screen bg-[#052e2b] p-4 text-white lg:p-6">
      <header className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-emerald-400 text-emerald-950"><ChefHat /></div>
          <div>
            <p className="font-black">DASHEM KDS</p>
            <p className="text-xs text-emerald-200">Fila de produção · terminal {deviceId.slice(-8)}</p>
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={() => void load()} aria-label="Atualizar fila" className="flex min-h-11 min-w-11 items-center justify-center rounded-xl border border-emerald-700">
            <RefreshCw className={`h-5 w-5 ${loading ? 'animate-spin' : ''}`} />
          </button>
          <button onClick={signOut} className="flex min-h-11 items-center gap-2 rounded-xl border border-emerald-700 px-4 text-sm font-bold">
      ...
      <section className="mx-auto mt-6 max-w-7xl">
        <div className="flex flex-wrap items-center gap-2">
          <button
            onClick={() => setPointId('')}
            className={`min-h-11 rounded-full px-4 text-xs font-black ${!pointId ? 'bg-emerald-400 text-emerald-950' : 'bg-emerald-900'}`}
          >
            Todos
          </button>
          {points.map((point) => (
            <button
              ...
              className={`min-h-11 rounded-full px-4 text-xs font-black
                ${pointId === point.id ? 'bg-emerald-400 text-emerald-950' : 'bg-emerald-900'}
                ${!point.is_active ? 'opacity-50' : ''}`}
            >
              {point.name}{!point.is_active ? ' · indisponível' : ''}
            </button>
          ))}
        </div>

        {error && <p className="mt-4 rounded-xl border border-amber-500 bg-amber-950/60 p-3 text-sm text-amber-100">{error}</p>}
        {loading && tickets.length === 0 && <Loader2 className="mx-auto mt-20 animate-spin" />}
        {!loading && tickets.length === 0 && (
          <div className="mt-16 rounded-3xl border border-dashed border-emerald-700 p-12 text-center text-emerald-200">
            <ChefHat className="mx-auto h-10 w-10" />
            <p className="mt-4 font-black">Fila vazia</p>
            <p className="mt-1 text-sm">Nenhuma comanda fictícia é exibida. Tickets surgem somente após dispatch persistido.</p>
          </div>
        )}

        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {tickets.map((projection) => (
            <article key={projection.ticket.id} className="rounded-3xl bg-white p-5 text-slate-950 shadow-xl">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="text-xs font-black uppercase tracking-wider text-emerald-700">{projection.point.name}</p>
                  <p className="mt-1 font-mono text-xs text-slate-500">#{projection.ticket.order_id.slice(0, 8)}</p>
                </div>
              ...
              {nextStatus[projection.ticket.status] && (
                <button
                  disabled={busy === projection.ticket.id}
                  onClick={() => void transition(projection)}
                  className="mt-5 min-h-12 w-full rounded-xl bg-emerald-600 text-sm font-black text-white disabled:opacity-50"
                >
                  {busy === projection.ticket.id ? 'Atualizando...' : labels[projection.ticket.status]}
                </button>
              )}
            </article>
          ))}
