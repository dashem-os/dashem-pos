...
export default function TablesShell() {
  return <OperationalSessionGate>{(selection) => <SelectedTablesShell selection={selection} />}</OperationalSessionGate>
}
...
function TablesSurface() {
  ...
  return <div className="min-h-screen bg-slate-100">
    <Toast toast={toast} />
    <nav className="flex min-h-14 flex-wrap items-center justify-between gap-3 py-2 border-b border-slate-200 bg-white px-4 sm:px-6"><div><p className="text-sm font-black text-slate-950">DASHEM <span className="text-orange-500">MESAS</span></p><p className="text-xs font-bold text-slate-500">{tenant?.name} · {store?.name}</p></div><div className="flex gap-2"><button onClick={() => navigateTo('/pos')} className="flex h-9 items-center gap-2 rounded-xl bg-rose-600 px-3 text-xs font-black text-white"><ShoppingCart className="h-4 w-4" />PDV</button><button onClick={signOut} className="flex h-9 items-center gap-2 rounded-xl border border-slate-300 px-3 text-xs font-black text-slate-600"><LogOut className="h-4 w-4" />Sair</button></div></nav>
