...
| 14 | Gestor tenta mutação no POS sem assunção | acesso negado | **FAIL na primeira execução**: com identidade administrativa e caixa fechado, informar R$ 100,00 abriu o turno. O produto autorizava por permissão, sem exigir sessão operacional. Decisão do dono do SaaS em 03/09/2026: seguir a matriz para abertura e fechamento. Corrigido no commit `f8d6246`. **Reexecução no deploy**: o fechamento foi recusado com "Fechar o caixa exige uma sessão operacional". A abertura foi reexecutada em 03/09/2026 22:42 e a interface **não oferece mais a ação** à identidade administrativa: em vez do campo de saldo inicial, o gestor recebe "O turno pertence a quem o assume. Você está conferindo a configuração com identidade administrativa; abrir e fechar o caixa exige código e PIN pessoal no terminal". | PASS. Vale registrar que a recusa não depende de a interface esconder o controle: o servidor recusa a chamada de qualquer origem, provado em `tests/test_cash_shift_authority.py`, que também separa a recusa por falta de turno da recusa por caixa inexistente |
