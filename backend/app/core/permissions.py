...
def route_requirement(method: str, path: str) -> RouteRequirement:
    ...
    if path in {
        "/api/v1/operational-access/session/end",
        "/api/v1/operational-access/session/heartbeat",
        "/api/v1/operational-access/session/context",
    }:
    ...
    if path.startswith("/api/v1/management"):
        if path.endswith("/productivity/rebuild"):
            return RouteRequirement("bi.refresh")
        if "/bi/" in path:
